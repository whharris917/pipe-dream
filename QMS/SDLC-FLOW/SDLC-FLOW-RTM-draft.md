...
revision_summary: 'CR-111 (v0.2): Addressed v0.1 review feedback. REQ-FS-ARCH-001
  evidence now acknowledges Scene.paint_particles / erase_particles as a documented
  authorized exception that mutates Simulation state directly via the time-travel
  snapshot pattern (per CLAUDE.md §3.3, exposed on ToolContext at lines 383-413).
  REQ-FS-ARCH-002 evidence corrected: the actual deletion command class is RemoveEntityCommand
  (model/commands/geometry.py:77), not DeleteEntityCommand. REQ-FS-CAD-001 constraint
  examples narrowed to verified class names from model/constraints.py (Coincident,
  Collinear, Length, Radius, EqualLength, Angle, FixedAngle, Midpoint). REQ-FS-CAD-002
  evidence reframed: solve() is a dispatcher taking use_numba and delegating to either
  the inline legacy OOP path or solve_numba() (not a peer of solve_numba). REQ-FS-CAD-003
  evidence corrected: tools mutate self.sketch.interaction_data directly at flow-state/ui/tools.py
  (the ctx.set_interaction_data facade method exists but is unused). REQ-FS-SIM-001
  evidence: integrate_n_steps spans lines 100-251 (corrected from 100-180); particle
  capacity is self.capacity (default 5000), not MAX_ATOMS; atom_color is (capacity,
  3). Qualified baseline unchanged at flow-state/main@a26f7fb.'
---
...
**Evidence:** `flow-state/ui/tools.py` (Select tool) injects mouse drag state by mutating `self.sketch.interaction_data` directly (e.g., line 788 clears it on tool-discard; analogous direct writes at lines ~950, ~978, ~1061 set it during drag). A `set_interaction_data(...)` / `clear_interaction_data(...)` facade pair exists on `ToolContext` at `flow-state/core/tool_context.py:427-458` but is not currently used by tools; tools reach into the Sketch directly. (`interaction_data` lives on Sketch — see REQ-FS-ARCH-003 known-asymmetry note — and the data is transient solver-target buffer, not persistent state, so the direct mutation is consistent with the Air Gap principle as it applies to persistent state.) The solver at `flow-state/model/solver.py` reads `interaction_data` each iteration (`Solver._solve_interaction` at line 42) and treats the mouse target as a constraint with effectively infinite mass — the cursor position is applied as a hard constraint, and other geometric constraints (length, parallel, anchored, etc.) negotiate around it via PBD. This produces emergent physical behaviors: dragging an end of an anchored line causes the line to *rotate about its anchor* rather than translate; dragging a midpoint of a line with two anchored endpoints causes the line to *deform within length-preservation limits*. The solver runs every frame as long as either constraints exist OR the user is interacting (per the orchestration loop in `core/scene.py` `update()`).
