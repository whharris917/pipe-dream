...
- `agent-hub/agent_hub/hub.py` -- Wire PTY Manager into agent start/stop lifecycle; rate-based activity filter
- `agent-hub/agent_hub/container.py` -- Add `get_container_id()` method for discovery
...
| EI-4 | Update README, run manual integration tests | Updated README: added `pty_manager.py` to architecture tree, `attach` to CLI table, `HUB_PTY_BUFFER_SIZE` to env vars, new PTY Manager section with description and CLI attach example. Updated CLI command count 5 to 6. | Pass | claude - 2026-02-08 |
| EI-5 | UAT: 10-test integration suite against live Hub | Executed full UAT against live Docker environment (2 authenticated agents: qa, claude). 9/10 tests PASS, 1 PARTIAL PASS (Test 4: idle timeout). Discovered F-001 (tmux status bar noise defeats idle detection) and F-002 (discovery missing container_id). Full results in `uat-cr066-results.md`. | Pass | claude - 2026-02-08 |
| EI-6 | Fix F-001: rate-based activity filter | Added `_pty_last_event` dict and rate-based filter to `Hub._on_pty_output()`. Only updates `last_activity` when two PTY events arrive within 10s (sustained output). tmux status bar (~60s interval) filtered; real Claude Code output (many events/s) passes immediately. Verified: 5 readings over 75s stable; 2-min idle timeout fired after 3m. | Pass | claude - 2026-02-08 |
| EI-7 | Fix F-002: capture container_id in discovery | Added `ContainerManager.get_container_id()`. Updated `_discover_running_containers()` and `_container_sync_loop()` to populate `agent.container_id`. Verified: API shows full container_id after discovery. | Pass | claude - 2026-02-08 |
| EI-8 | Test 5 follow-up: manual CLI attach verification | Lead manually ran `agent-hub attach qa`, confirmed tmux session rendered correctly, Claude Code interface visible, `Ctrl-B D` clean detach, agent remained running after detach. | Pass | Lead - 2026-02-08 |
...
|---------|---------------------|
| All modules import cleanly (`python -c "from agent_hub.hub import AgentHub"` etc.). | claude - 2026-02-08 |
| UAT executed against live Docker environment. Two findings discovered and resolved: F-001 (tmux status bar noise) fixed with rate-based activity filter; F-002 (missing container_id on discovery) fixed with new `get_container_id()` method. All 10 tests now pass. CLI attach manually verified by Lead. | claude - 2026-02-08 |
...
All eight EIs completed as planned. The PTY Manager module (`pty_manager.py`) provides a clean interface between the Docker exec socket layer and future WebSocket consumers: `attach`/`detach` for lifecycle, `register_callback` for output streaming, `get_buffer` for scrollback, `write`/`resize` for input. Hub integration wires PTY sessions into the full agent lifecycle including discovery of pre-existing containers and the container sync loop. The `attach` CLI command provides immediate usability.

Live UAT (10 tests) executed against Docker environment with two agents (qa, claude). Two findings discovered during UAT: tmux status bar noise defeating idle detection (F-001) and missing container_id on discovery (F-002). Both findings were resolved and re-verified in the same session. CLI attach manually verified by Lead. Final result: all 10 tests pass.
