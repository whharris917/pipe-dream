...
|----|------------------|-------------------|--------------|---------------------|
| EI-1 | Hub auto-starts MCP servers: Add `ensure_mcp_servers(config)` call in `cli.py` `start()` command | Added import of `ensure_mcp_servers` from `agent_hub.services` at line 13. Inserted `ensure_mcp_servers(config)` call between `HubConfig` creation and `AgentHub` instantiation (line 42). Function is idempotent -- checks `is_port_alive()` before starting. | Pass | claude - 2026-02-09 |
| EI-2 | Configure Tauri shell scope: Add scoped `shell:allow-spawn` permission to `default.json` | Original plan used Tauri shell plugin with `shell:allow-spawn`. During UAT, replaced with a Rust `spawn_hub` Tauri command in `lib.rs` for detached process spawning (see execution comments). `default.json` reverted to original -- shell scope not needed. | Pass | claude - 2026-02-09 |
| EI-3 | GUI auto-bootstrap: Create `ensureHub.ts`, modify `types.ts`, `App.tsx`, `StatusBar.tsx`, `global.css` | Created `ensureHub.ts` with `isHubAlive()` and `ensureHub()` functions using `invoke("spawn_hub")` for detached process spawning. Added `"bootstrapping"` to `ConnectionStatus` union in `types.ts`. Replaced `useHubConnection()` hook in `App.tsx` with explicit `ensureHub().then(hubConnection.connect)` lifecycle using `useRef` guard. Added "Starting Hub..." label in `StatusBar.tsx`. Added `.bootstrapping` CSS class with yellow pulsing dot in `global.css`. Additional files: `lib.rs` (Rust `spawn_hub` command), `config.py` (`_find_project_root()` auto-detection). | Pass | claude - 2026-02-09 |
| EI-4 | Fix terminal DA escape sequence: Add `muteInput` guard to `useHubConnection.ts` | Added `private muteInput = new Set<string>()` field to `HubConnection`. In the "subscribed" dispatch case, added `muteInput.add(agentId)` before `term.write()` and `muteInput.delete(agentId)` in the write callback. Added early return in `sendInput()` when agent is in `muteInput`. Added regex filter for DA responses (`/\x1b\[[\?>][\d;]*c/` and `/^\[[\?>][\d;]*c$/`) as defense-in-depth. | Pass | claude - 2026-02-09 |
| EI-5 | Update `agent-hub/README.md`: Fix stale refs, document auto-start and GUI | Fixed `services` -> `status` in Quick Start and Primary Commands table. Updated `agent-hub start` description to note MCP auto-start. Updated gui description to mention auto-bootstrap. | Pass | claude - 2026-02-09 |
| EI-6 | Update `agent-hub/gui/README.md`: Document auto-bootstrap, update prerequisites | Replaced Hub prerequisite with "Agent Hub CLI installed and venv active". Added `ensureHub.ts` to architecture tree. Added "Auto-Bootstrap" section documenting the startup sequence. Updated Key Design Decisions with "Demand-driven bootstrap" and "Buffer replay muting". | Pass | claude - 2026-02-09 |
| EI-7 | UAT: Execute all test scenarios from Section 8 | All 6 test scenarios passed. Test 1: GUI bootstrapped Hub from clean state, "Starting Hub..." -> "Connected". Test 2: Hub survived GUI close (detached process), immediate "Connected" on relaunch. Test 3: No DA escape text on initial open or tab re-open. Test 4: `agent-hub start` auto-started MCP servers before Hub (406 responses confirm servers alive). Test 5: `agent-hub launch claude` started full stack cleanly. Test 6: Venv deactivated, GUI stayed at "Disconnected" with no crash. | Pass | Lead + claude - 2026-02-09 |
...
|---------|---------------------|
| EI-1 through EI-6 implemented per plan. EI-7 (UAT) deferred to Lead for manual verification on running system. | claude - 2026-02-09 |
| UAT Test 1 revealed that `agent-hub start` fails when run from a subdirectory (e.g., `agent-hub/gui/`) because `HubConfig` defaults `project_root` to `Path.cwd()`. Tauri spawns the process with cwd set to the GUI directory. Fixed by adding `_find_project_root()` to `config.py` which walks up from cwd looking for `QMS/` + `.claude/` marker directories. Additional file modified: `agent-hub/agent_hub/config.py`. | claude - 2026-02-09 |
| UAT Test 2 revealed that closing the GUI kills the Hub because `Command.spawn()` via the Tauri shell plugin creates a child process tied to the GUI's process group. Fix: replaced shell plugin approach with a Rust `spawn_hub` Tauri command in `lib.rs` that spawns the Hub as a fully detached process (`CREATE_NEW_PROCESS_GROUP` + `CREATE_NO_WINDOW` on Windows, `setsid` on Unix). Updated `ensureHub.ts` to use `invoke("spawn_hub")` instead of `Command.create()`. Reverted `default.json` to original (shell:allow-spawn no longer needed). EI-2 approach changed; EI-3 updated accordingly. Additional file modified: `agent-hub/gui/src-tauri/src/lib.rs`. | claude - 2026-02-09 |
...
All 7 execution items completed with Pass outcomes. Two deviations from the original plan discovered during UAT:

1. **Project root auto-detection** (discovered in Test 1): `HubConfig` defaulted `project_root` to `Path.cwd()`, causing failures when `agent-hub start` was invoked from subdirectories (including GUI spawn). Fixed by adding `_find_project_root()` to `config.py`. Additional file: `agent-hub/agent_hub/config.py`.

2. **Detached process spawning** (discovered in Test 2): The Tauri shell plugin's `Command.spawn()` creates child processes tied to the GUI's process group, killing the Hub on GUI close. Replaced with a Rust `spawn_hub` Tauri command using `CREATE_NEW_PROCESS_GROUP | CREATE_NO_WINDOW` (Windows) / `setsid` (Unix). The `shell:allow-spawn` scope from EI-2 was reverted as unnecessary. Additional file: `agent-hub/gui/src-tauri/src/lib.rs`.

Final file list (12 files modified/created vs. 10 planned):
- `agent-hub/agent_hub/cli.py` (EI-1)
- `agent-hub/agent_hub/config.py` (UAT fix)
- `agent-hub/gui/src-tauri/capabilities/default.json` (EI-2, reverted)
- `agent-hub/gui/src-tauri/src/lib.rs` (UAT fix)
- `agent-hub/gui/src/ensureHub.ts` (EI-3, new)
- `agent-hub/gui/src/types.ts` (EI-3)
- `agent-hub/gui/src/App.tsx` (EI-3)
- `agent-hub/gui/src/components/StatusBar.tsx` (EI-3)
- `agent-hub/gui/src/styles/global.css` (EI-3)
- `agent-hub/gui/src/hooks/useHubConnection.ts` (EI-4)
- `agent-hub/README.md` (EI-5)
- `agent-hub/gui/README.md` (EI-6)
