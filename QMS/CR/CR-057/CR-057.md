...
| EI-7 | Update `docker/README.md` with stdio proxy architecture documentation | Added "MCP Architecture: Stdio Proxy" section explaining the proxy design, protocol flow, configuration example, and rationale. Updated filesystem diagram (.mcp.json description), troubleshooting section (proxy-specific debugging), and references (added CR-057). | Pass | claude - 2026-02-06 |
...
All 7 EIs completed successfully. The stdio-to-HTTP proxy (`mcp_proxy.py`) replaces direct HTTP MCP transport in containers, eliminating the ~10% intermittent connection failure. One unplanned discovery during implementation: MCP streamable-http servers return SSE (`text/event-stream`) format, not plain JSON — the proxy was updated to handle both formats. Reliability testing achieved 40/40 (100%) across scripted and manual tests, up from the previous best of 27/30 (90%) with HTTP transport.
