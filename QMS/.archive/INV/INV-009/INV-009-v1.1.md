...
revision_summary: All CAPAs verified complete via CR-050; qms-cli requalified with
  commit 63123fe (364/364 tests passing)
---
...
|------|------|-------------|----------------|---------|---------------------|
| INV-009-CAPA-001 | Corrective | Create a new CR to: (1) fix the failing test (test_invalid_user), (2) verify CI passes on dev branch before merge, (3) merge to main and verify CI passes on main, (4) update SDLC-QMS-RTM with the verified commit hash | CR-050 created and executed. Test fixed in tests/test_qms_auth.py line 136. CI run 21609392809 passed (364/364 tests). SDLC-QMS-RTM v8.1 updated with commit 63123fe. | Pass | qa - 2026-02-02 |
| INV-009-CAPA-002 | Preventive | Configure CI to run on all branches | CR-050 EI-1: Updated .github/workflows/tests.yml to trigger on branches: ['**'] | Pass | qa - 2026-02-02 |
| INV-009-CAPA-003 | Preventive | Add GitHub branch protection requiring CI to pass before merge | CR-050 EI-4: Configured via gh api with required_status_checks strict=true, contexts=["test"] | Pass | qa - 2026-02-02 |
| INV-009-CAPA-004 | Preventive | Update RTM review checklist to require verification that commit hash is not "TBD" | CR-050 EI-5: Added to CLAUDE.md Post-Flight checklist and qa.md Review Criteria table | Pass | qa - 2026-02-02 |
| INV-009-CAPA-005 | Preventive | CI must run ALL tests (not just qualification/) - any failure blocks qualification | CR-050 EI-1: Updated tests.yml to run `pytest tests/` instead of `pytest tests/qualification/` | Pass | qa - 2026-02-02 |
...
|---------|---------------------|
| Investigation initiated after Lead discovered CI failures on qms-cli main branch. | claude - 2026-02-02 |
| CR-050 created to implement all five CAPAs. | claude - 2026-02-02 |
| CR-050 EI-2 initially blocked by GitHub Actions infrastructure issues (runner not acquired). Resolved by re-triggering CI. | claude - 2026-02-02 |
| CR-050 closed after successful post-approval review. All CAPAs verified complete. | qa - 2026-02-02 |

---

## 9. Execution Summary

All five CAPAs have been successfully implemented and verified via CR-050:

**Corrective Action (CAPA-001):**
- Fixed failing test_invalid_user assertion
- CI verified on both dev branch and main (364/364 tests passing)
- SDLC-QMS-RTM v8.1 updated with commit 63123fe

**Preventive Actions (CAPA-002 through CAPA-005):**
- CI now triggers on ALL branches (not just pre-defined list)
- GitHub branch protection requires CI to pass before merge to main
- RTM review checklist updated to catch "TBD" or placeholder commit hashes
- CI runs full test suite (364 tests), not just qualification subset (166 tests)

**Impact Resolution:**
- qms-cli is now properly qualified with CI-verified commit 63123fe
- The process gaps that allowed the original deviation have been closed
- Future CRs cannot merge to main without passing CI
