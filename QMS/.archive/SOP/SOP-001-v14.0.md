...
revision_summary: 'CR-017: Add TEMPLATE document type'
---
...
- `qms --user {USER} status {DOC_ID}` — Current version, status, responsible user
- `qms --user {USER} history {DOC_ID}` — Complete revision history with timestamps
...
```
qms --user {USER} route {DOC_ID} --approval --retire
```
...
| Command | Description |
|---------|-------------|
| `qms --user {USER} create {TYPE} [--title "Title"]` | Create a new document |
...
| `qms --user {USER} status {DOC_ID}` | Show document status from .meta |

### 13.2 Workflow Commands

| Command | Description |
|---------|-------------|
| `qms --user {USER} route {DOC_ID} --review` | Route for review (pre/post inferred from status) |
| `qms --user {USER} route {DOC_ID} --approval` | Route for approval (pre/post inferred from status) |
| `qms --user qa assign {DOC_ID} --assignees {users}` | Add reviewers (QA only) |
| `qms --user {USER} review {DOC_ID} --recommend --comment "{text}"` | Submit positive review |
| `qms --user {USER} review {DOC_ID} --request-updates --comment "{text}"` | Request changes |
| `qms --user {USER} approve {DOC_ID}` | Approve document |
| `qms --user {USER} reject {DOC_ID} --comment "{text}"` | Reject document |
| `qms --user {USER} release {DOC_ID}` | Release for execution |
| `qms --user {USER} revert {DOC_ID} --reason "{text}"` | Revert to execution |
| `qms --user {USER} close {DOC_ID}` | Close document |

### 13.3 Audit & History Commands

| Command | Description |
|---------|-------------|
| `qms --user {USER} history {DOC_ID}` | View full audit trail |
| `qms --user {USER} comments {DOC_ID}` | View review comments for current version |
| `qms --user {USER} comments {DOC_ID} --version N.X` | View comments for specific version |

### 13.4 User Commands

| Command | Description |
|---------|-------------|
| `qms --user {USER} inbox` | List tasks in user's inbox |
| `qms --user {USER} workspace` | List documents in user's workspace |

### 13.5 Document Lifecycle Commands

| Command | Description |
|---------|-------------|
| `qms --user {USER} cancel {DOC_ID} --confirm` | Permanently delete never-effective document (v < 1.0) |
| `qms --user {USER} route {DOC_ID} --approval --retire` | Route for retirement approval (leads to RETIRED) |
...
| `qms --user lead migrate [--dry-run]` | Migrate documents to new metadata architecture |
| `qms --user lead verify-migration` | Verify migration completed correctly |
...
|---------|------|--------|---------|
| 14.0 | 2026-01-09 | qa | — |
| 13.0 | 2026-01-08 | qa | CR-017: Add TEMPLATE document type |
| 12.0 | 2026-01-07 | qa | CR-015: Remove DS, CS, OQ; fix CAPA classification per SOP-006 |
| 11.0 | 2026-01-07 | qa | CR-014: Add VAR to controlled document types, definitions, and naming conventions |
| 10.0 | 2026-01-04 | qa | CR-006: Grant QA route permission in Section 4.2 |
| 9.0 | 2026-01-04 | qa | CR-004: Require CR ID in revision_summary for traceability (Section 5.1) |
| 8.0 | 2026-01-04 | qa | Released as effective |
| 7.0 | 2026-01-03 | qa | Released as effective |
| 6.0 | 2026-01-03 | qa | Released as effective |
| 5.0 | 2026-01-03 | qa | Released as effective |
