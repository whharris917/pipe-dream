...
revision_summary: 'CR-085: Add pre-execution commit step to Section 7.2'
---
...
2. **Integration verification:** How the changed functionality will be exercised through user-facing levers in a running system to demonstrate the change is effective

Integration verification is a practical demonstration that the change works as intended when a user interacts with the system -- not a formal test protocol. Its purpose is to catch problems that automated tests with narrow mocks or overly specific assertions might miss.

For document-only CRs, a description of procedural verification is sufficient.

Examples:
- "Manual verification of corrected frontmatter fields" (document-only)
- "Unit tests for solver convergence; launch application and verify constrained geometry behaves correctly when dragged interactively" (code CR)
- "CI passes all 403 tests; launch application and verify new toolbar renders correctly across supported resolutions" (code CR)
...
1. Commits and pushes the QMS repository to capture the pre-execution state (per SOP-004 Section 5)
2. Releases the CR for execution (`qms release`)
