...
revision_summary: 'CR-115: Add Exploratory CR notes to §6.2 (mandate four bounds —
  target submodule, execution branch, RS-immutability declaration, anti-scope) and
  §7.1 (QA may forgo TU assignment at pre-review when CR is Exploratory and bounds
  are present; post-review TU assignment is mandatory and determined by actual diff).
  Cross-references QMS-Policy §6 (paragraph added by same CR) and qa.md ## Exploration
  CRs section.'
---
...
**Exploratory CRs.** When a CR is intentionally loose-scope (identified as "Exploratory" in §1 Purpose per QMS-Policy §6), the Scope section MUST include the following bounds:
