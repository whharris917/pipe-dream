...
revision_summary: 'CR-082: Add ADD (Addendum) document type to Sections 2, 3, 6.1,
  8.2'
---
...
| **Executable Document** | A document that authorizes implementation activities (CR, INV, TP, ER, VAR, ADD) |
...
| **Addendum Report (ADD)** | Child document created to correct or supplement a closed executable document; must reference a CLOSED parent (see SOP-004 Section 9B) |
...
### 8.2 Executable Documents (CR, INV, TP, ER, VAR, ADD)
...
```
python qms-cli/qms.py --user qa assign {DOC_ID} --assignees tu_ui tu_scene   # QA assigns additional reviewers
```
