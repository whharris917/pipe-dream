...
revision_summary: 'CR-040: Replace specific user/agent identifiers with generic terminology'
---
...
- All spawned subagents (Quality agents, Reviewer agents, or other subagents defined for the project)
...
| **Subagent** | A spawned agent assigned a specific role (e.g., Quality agent, Reviewer agent, or other project-defined roles) |
...
4. Report anomalies or concerns to the orchestrator or project authority
...
3. NOT spawn fresh agents of the same type unless explicitly directed by the project authority

There is no valid rationale for "refreshing context" by spawning new agents. If context refresh is required, the project authority will direct it explicitly.
...
4. Access files outside the project directory without project authority authorization
...
2. Escalated to the project authority for resolution
...
| Investigation (INV) | Only if the project authority determines one is warranted |

The audit trail provides sufficient visibility into agent behavior. Formal investigation is reserved for significant or systemic issues at the project authority's discretion.
