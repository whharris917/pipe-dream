...
revision_summary: Address QA review deficiencies -- remove hub.py from scope (Section
  2.3), add execution phase instructions (Section 10), fix Unicode encoding
---
...
Add a readiness check to the Agent Hub's container startup that polls `tmux capture-pane` output until Claude Code's input prompt character (U+276F) is detected, ensuring notifications are only injected after the agent is ready to accept input.

### 2.3 Files Affected

- `agent-hub/agent_hub/container.py` -- Add `_wait_for_ready()` method using `tmux capture-pane`
...
After `_exec_claude()`, a new `_wait_for_ready()` method polls `tmux capture-pane -t agent -p` via `docker exec` until the Claude Code prompt character (U+276F) appears in the output. If the prompt is detected, `start()` returns normally and the agent becomes RUNNING. If the timeout expires without detecting the prompt, the method logs a warning and raises `RuntimeError`. The existing error handler in `start_agent()` sets the agent to ERROR state but does not stop or remove the container -- it remains alive for the Lead to attach and investigate. This ensures that when `agent.state` becomes `RUNNING`, Claude Code is genuinely ready to accept input, and notification injection will succeed.
...
- Verify `tmux capture-pane` reliably detects the Claude Code prompt (U+276F) during startup
...
|----|------------------|-------------------|--------------|---------------------|
| EI-1 | Add `_wait_for_ready()` to `container.py` and call from `start()` | Added `_wait_for_ready()` method (lines 244-275) that polls `tmux capture-pane` for U+276F prompt character with 60s timeout and 2s polling interval. Inserted call in `start()` after `_exec_claude()` (line 67). On timeout, raises RuntimeError; existing `start_agent()` error handler in hub.py sets agent to ERROR state while leaving container alive for inspection. | Pass | claude - 2026-02-08 |
| EI-2 | Integration test: verify notification injection succeeds after auto-start | Restarted Hub to load new code. Set QA policy to `auto_on_task`. Created test CR-066 (raw template) and routed for review. Hub detected inbox change, auto-launched QA container. QA state transitioned: stopped -> starting -> running. `tmux capture-pane` confirmed: (1) Claude Code prompt appeared, (2) notification text "CR-066 review is in your inbox" was injected AND submitted, (3) QA began orchestrating the review task. Test CR-066 canceled after verification. | Pass | claude - 2026-02-08 |
...
|---------|---------------------|
| Test CR-066 created as a raw template vehicle to trigger QA inbox. Successfully triggered auto-launch. CR-066 canceled after test verification. | claude - 2026-02-08 |
...
Both execution items completed successfully. Added `_wait_for_ready()` readiness check to `container.py` and verified via integration test that notifications are now injected only after Claude Code is ready to accept input. The timing bug is resolved: Hub-launched agents reliably receive and process their first task notification.
