...
| EI-2 | Configure Tauri shell scope: Add scoped `shell:allow-spawn` permission to `default.json` | Added `shell:allow-spawn` permission object to the permissions array in `default.json`. Scoped to a single allowed command: `agent-hub-start` mapping to `agent-hub` with args `["start"]`. | Pass | claude - 2026-02-09 |
| EI-3 | GUI auto-bootstrap: Create `ensureHub.ts`, modify `types.ts`, `App.tsx`, `StatusBar.tsx`, `global.css` | Created `ensureHub.ts` with `isHubAlive()` and `ensureHub()` functions. Added `"bootstrapping"` to `ConnectionStatus` union in `types.ts`. Replaced `useHubConnection()` hook in `App.tsx` with explicit `ensureHub().then(hubConnection.connect)` lifecycle using `useRef` guard. Added "Starting Hub..." label in `StatusBar.tsx`. Added `.bootstrapping` CSS class with yellow pulsing dot in `global.css`. | Pass | claude - 2026-02-09 |
| EI-4 | Fix terminal DA escape sequence: Add `muteInput` guard to `useHubConnection.ts` | Added `private muteInput = new Set<string>()` field to `HubConnection`. In the "subscribed" dispatch case, added `muteInput.add(agentId)` before `term.write()` and `muteInput.delete(agentId)` in the write callback. Added early return in `sendInput()` when agent is in `muteInput`. | Pass | claude - 2026-02-09 |
...
| EI-7 | UAT: Execute all test scenarios from Section 8 | Deferred to Lead for manual UAT on the running system. Code changes verified through file review. | [Pass/Fail] | [PERFORMER] - [DATE] |
