...
revision_summary: Section 8 closure completed per QA post-review feedback
---
...
| EI-4 | Manual verification: launch agent, verify scrollback works, verify input works, verify resize works | Lead launched GUI (`npm run tauri dev`), started BU agent, and verified: (1) **Input works** — sent messages to BU and received responses in the GUI terminal tab. (2) **Scrollback works** — conversation history preserved and scrollable. (3) **Resize works** — window resize produces clean re-render at correct width with no duplication. Initial testing revealed three duplication issues (startup banner doubled, resize content doubled, boundary artifacts) caused by Ink's full TUI re-render appending below stale content. Fixed by tracking pending re-renders: subscribe clears viewport before resize (with 1.5s ring buffer fallback), and first output after any resize calls `term.clear()` to wipe viewport + scrollback so the re-render (which contains the complete conversation) replaces all stale content. ESC[3J also converted to ESC[2J in `stripTerminalModes()` to preserve scrollback during normal operation. All verification criteria met after fix. | Pass | claude - 2026-02-15 |
...
| EI-4 required three debugging iterations: (1) CRLF line endings causing invisible log output — fixed with `.rstrip('\r')`. (2) No initial screen content — fixed by sending `capture-pane -p -e -t %0` after attach with `%begin/%end` block parser. (3) Unicode box-drawing chars crashing `_decode_octal()` — fixed by using `ch.encode('utf-8')` instead of `ord(ch)`. After core functionality verified, three duplication issues identified and resolved in `useHubConnection.ts` via pending re-render tracking and `term.clear()`. Final test file: 32 unit tests (14 decode_octal including Unicode, 11 decode_control_output, 7 encode_send_keys), all passing. | claude - 2026-02-15 |
...
|-----------------------|---------|---------------------|
| PTY manager rewritten to use tmux control mode (`tmux -CC`), bypassing viewport rendering entirely. GUI simplified to strip only mouse tracking and alt-screen modes; ESC[3J converted to ESC[2J to preserve scrollback. Re-render duplication on subscribe and resize resolved via pending re-render tracking with `term.clear()`. 32 unit tests added, all passing alongside 10 existing UAT tests. Manual verification confirmed input, scrollback, and resize all working correctly. | Resolved — all EIs Pass | claude - 2026-02-15 |
