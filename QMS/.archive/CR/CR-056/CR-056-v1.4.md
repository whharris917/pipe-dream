...
revision_summary: Final execution with all three VARs referenced and resolved
---
...
| Final end-to-end test performed using `launch.sh`, a unified container launcher that supersedes the five previous scripts. `launch.sh` uses a `docker run -d -e SETUP_ONLY=1` + `docker exec -it` pattern: entrypoint runs setup then sleeps, interactive session attaches separately. Command tested: `./launch.sh claude qa`. Result: MCP servers auto-started, both containers launched in separate mintty windows, both agents connected to MCP (100% reliability via stdio proxy), correct agent identities confirmed (claude uses real CLAUDE.md, QA uses overlay), test CR routed, inbox watcher detected new file, QA confirmed inbox task via MCP. No re-authentication required. Lead confirmed this as the definitive final proof of CR-056's objectives. | claude + lead - 2026-02-06 |
| Three VARs filed to maintain GMP compliance for scope deviations discovered during execution: (1) CR-056-VAR-001 (Type 1, CLOSED): MCP reliability gap, resolved by CR-057 stdio proxy achieving 100% reliability. (2) CR-056-VAR-002 (Type 1, CLOSED): `launch.sh` supersedes the scripts named in pre-approved scope; QA reviewed and approved the naming deviation, obsolete scripts deleted. (3) CR-056-VAR-003 (Type 2, PRE_APPROVED): inbox watcher detects files but does not yet inject notifications into agent containers per Section 5.6; follow-up CR to be created. All blocks on CR-056 closure are cleared. | claude - 2026-02-06 |

---

## 11. Execution Summary

All seven execution items were implemented and tested in Session 2026-02-04-003. The multi-agent system was demonstrated end-to-end: claude and qa launched in separate terminal windows, both agents connected to MCP servers, QA correctly self-identified (after fixing a `docker-compose run` volume mount bug), and a QMS workflow test was conducted successfully.

However, the Lead observed that MCP auto-connect is intermittently unreliable on subsequent container launches (~10% failure rate per server). Rather than close the CR with a known instability, the Lead halted execution pending investigation. Session 2026-02-05-001 was consumed by a 17-phase debugging effort that identified five root causes and fixed four, improving reliability from near-0% to 90%. The fifth root cause (Claude Code's HTTP MCP client intermittently failing to initiate connections inside Docker) cannot be fixed from the server side. Significant infrastructure hardening was performed during debugging: entrypoint rewrite (IPv4-first DNS, jq-based MCP state cleaning), Dockerfile improvements (jq package), MCP config fixes (auth headers), and server-side improvements (`stateless_http=True` on both MCP servers).

Three variances were filed to maintain GMP compliance as the execution evolved beyond the pre-approved scope:

**CR-056-VAR-001 (Type 1, CLOSED):** MCP auto-connect reliability. Corrective action: CR-057 implemented a stdio-to-HTTP proxy inside the container, eliminating HTTP transport from Claude Code's perspective entirely. Result: 100% MCP connection reliability (40/40 in combined testing), up from 90% with HTTP. The definitive end-to-end retest (VAR-001 EI-3) confirmed all CR-056 objectives met with full reliability.

**CR-056-VAR-002 (Type 1, CLOSED):** Launcher script naming deviation. During execution, the iterative development of `claude-session.sh` produced four variants, and the multi-agent `multi-agent-session.sh` was also created. After CR-057 resolved MCP reliability, all five scripts were superseded by a single unified launcher: `launch.sh`. VAR-002 formally authorized this deviation -- QA reviewed and approved through both pre- and post-approval cycles, confirming that `launch.sh` satisfies every functional requirement from the pre-approved scope. The five obsolete scripts were deleted as part of VAR-002 execution.

**CR-056-VAR-003 (Type 2, PRE_APPROVED):** Inbox watcher notification injection. The inbox watcher (`docker/scripts/inbox-watcher.py`) correctly detects new inbox files and prints notifications to its own monitoring terminal, but does not yet inject notifications into agent containers via `docker exec` as described in Sections 4, 5.6, and T4. This is a convenience gap, not a blocking one -- the multi-agent workflow is fully operational. A follow-up CR will absorb the remaining injection work. Type 2 designation: pre-approval is sufficient to unblock CR-056 closure.

The final test result: `./launch.sh claude qa` launched both agents in separate terminal windows, MCP connected at 100% via stdio proxy, both agents self-identified correctly, a test CR was routed from claude to qa, the inbox watcher detected the new file, and QA confirmed the task via MCP tools. The Lead confirmed this as a complete success. All CR-056 core objectives -- multi-agent containers, agent identity isolation, reliable MCP connectivity, and inbox file detection -- are fully met. The inbox watcher's notification injection remains as tracked follow-up work (VAR-003).
...
- **CR-056-VAR-001:** MCP Auto-Connect Reliability Blocks Multi-Agent Testing (Type 1, CLOSED)
- **CR-056-VAR-002:** Unified launch.sh Replaces Named Scripts in Pre-Approved Scope (Type 1, CLOSED)
- **CR-056-VAR-003:** Inbox Watcher Notification Injection Not Implemented (Type 2, PRE_APPROVED)
- **CR-057:** MCP Stdio Proxy for Reliable Container MCP Connections (CLOSED; corrective action from VAR-001)
