...
|----|------------------|-------------------|--------------|---------------------|
| VAR-EI-1 | Create INV to investigate deviation and CR-042 qualification gap | INV-008 created to investigate both the unauthorized qms-cli modification and the CR-042 qualification gap. INV addresses two root causes: (1) inadequate verification testing for CR-042 SSE transport, (2) no separation between test and production code access. CAPAs: CAPA-001 creates requalification CR for SSE transport (produces CLI-5.0); CAPA-002 gates INV closure on containerization becoming operational. INV-008 pre-approved v1.0 on 2026-02-01. | Pass | claude - 2026-02-01 |
...
|---------|---------------------|
| INV-008 comprehensively addresses the scope handoff from this VAR: (1) deviation analysis documents the unauthorized modification and determines formal incorporation via requalification CR, (2) qualification gap analysis identifies RC-1 (inadequate verification testing) and RC-2 (no code access separation). No scope items lost in handoff. | claude - 2026-02-01 |
...
|-----------------------|---------|---------------------|
| INV-008 created and pre-approved (v1.0) to address the unauthorized qms-cli modification and CR-042 qualification gap. Per SOP-004 Section 9A.2, Type 2 VAR block release requires only pre-approval of the follow-up mechanism. INV-008 CAPAs will deliver: (1) requalification CR for SSE transport producing CLI-5.0, (2) containerization operational as preventive control. All resolution work complete; VAR ready for closure. | Pass | claude - 2026-02-01 |
