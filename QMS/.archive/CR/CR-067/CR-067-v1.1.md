...
|----|------------------|-------------------|--------------|---------------------|
| EI-1 | Create `broadcaster.py` — ConnectionState + Broadcaster with fan-out methods | Created `agent-hub/agent_hub/broadcaster.py` with ConnectionState (per-connection WebSocket + subscription set + unique ID), Broadcaster class with asyncio.Lock-protected connection management, broadcast_pty_output (subscribers only, base64), broadcast_agent_state/broadcast_inbox_change (all clients), safe_send with stale connection cleanup. Commit 2deaa6a. | Pass | claude - 2026-02-08 |
| EI-2 | Create `api/websocket.py` — FastAPI WebSocket route + message dispatch | Created `agent-hub/agent_hub/api/websocket.py` with `/ws` route. Handlers for subscribe (validates agent, sends scrollback buffer), unsubscribe, input (validates subscription, writes to PTY), resize (validates subscription). Error responses for missing fields, unknown agents, unsubscribed input. WebSocketDisconnect cleanup in finally block. Commit 2deaa6a. | Pass | claude - 2026-02-08 |
| EI-3 | Modify `api/server.py` — mount WebSocket route, create and wire Broadcaster | Modified `server.py`: imports ws_router and Broadcaster, creates Broadcaster on app.state in create_app(), wires via hub.set_broadcaster() in lifespan, mounts ws_router at root. Commit 2deaa6a. | Pass | claude - 2026-02-08 |
| EI-4 | Modify `hub.py` — set_broadcaster(), PTY callback registration, event emission | Modified `hub.py`: added `_broadcaster` attribute, `set_broadcaster()` registers as PTY callback, `_broadcast_state()` helper emits agent state changes from start_agent/stop_agent/_container_sync_loop, inbox events from _on_inbox_change, cleanup in stop(). TYPE_CHECKING import avoids circular dep. Commit 2deaa6a. | Pass | claude - 2026-02-08 |
| EI-5 | UAT — manual WebSocket testing per Section 8 | Created `tests/test_websocket_uat.py` with 10 tests using FastAPI TestClient: T1 connect/disconnect lifecycle, T2 subscribe known agent (subscribed + buffer), T3 subscribe unknown agent (error), T4 unsubscribe, T5 input without subscription (error), T6 resize without subscription (error), T7 unknown message type (error), T8 missing agent_id (error), T9 broadcast_agent_state to all clients, T10 broadcast_inbox_change to all clients. All 10 PASS (pytest 3.00s). | Pass | claude - 2026-02-08 |
| EI-6 | Update `agent-hub/README.md` with WebSocket API documentation | Updated architecture tree (added broadcaster.py, websocket.py), added full WebSocket API section with message protocol tables and example session, updated PTY Manager section (no longer "future"), updated dependencies. | Pass | claude - 2026-02-08 |
...
All 6 EIs pass. Implementation adds a WebSocket endpoint at `/ws` with a Broadcaster fan-out pattern that plugs into the existing PTY Manager callback interface. The message protocol uses JSON with base64-encoded PTY data. Hub integration emits agent state-change and inbox-change events to all connected WebSocket clients. UAT validates 10 protocol scenarios via FastAPI TestClient. No deviations from plan.
