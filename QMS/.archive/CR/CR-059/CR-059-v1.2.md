...
| EI-7 | Stop container, restart, verify `projects/` persists and MCP connects successfully | After image rebuild (`docker-compose build --no-cache`), container was stopped and restarted. Agent successfully read previous session transcripts from `projects/-pipe-dream/` directory — confirmed `projects/` survived restart. MCP servers connected successfully on restart. Agent demonstrated cross-session continuity by retrieving details from the previous session's JSONL transcript. | Pass | lead - 2026-02-07 |
...
| EI-7 retest after `docker-compose build --no-cache`: `projects/` directory persisted across restart. Agent read previous session JSONL transcripts and recalled details (NC State vs Virginia Tech basketball game from a prior session). MCP connectivity confirmed. | claude - 2026-02-07 |
