...
revision_summary: 'CR-075: Update REQ-MCP-009, -011, -014, -015, -016 and Section
  5.4 for header-based identity model'
---
...
| REQ-MCP-015 | **Header-Based Identity Resolution.** The MCP server shall resolve caller identity based on the presence of the `X-QMS-Identity` HTTP request header. When the header is present (enforced mode): identity shall be read from the header value; the `user` tool parameter shall be ignored. When the header is absent (trusted mode): identity shall be read from the `user` tool parameter. All MCP tools shall accept a `ctx: Context` parameter for request context access. |
