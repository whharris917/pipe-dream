...
| CAPA-003/004 executed via CR-100 (CLOSED v2.0). SOP-005 v7.0, TEMPLATE-CR v10.0, SOP-002 v16.0 all EFFECTIVE. Seed aligned via PR #18 (CI passed). | claude - 2026-02-22 |
...
All four CAPAs executed successfully. CAPA-001 corrected the deny rule patterns and discovered that all Claude Code deny rules are non-functional due to a platform bug; implemented a PreToolUse hook as the actual enforcement mechanism for all three protected directories (QMS/, qms-cli/, flow-state/). CAPA-002 retroactively verified 673 tests pass at the ungoverned commit. CAPA-003 and CAPA-004, bundled in CR-100, delivered SOP-005 v7.0 (development environment, PR mandate, file scope, direct commit prohibition), TEMPLATE-CR v10.0 (explicit locations, SOP-005 workflow reference, PR enforcement), and SOP-002 v16.0 (PR merge verification in QA post-review). The seed TEMPLATE-CR was updated via the full SOP-005 Section 7.1 execution branch workflow (PR #18, CI passed, 673 tests), validating the controls being established.
