...
revision_summary: 'All CAPAs complete: CAPA-003 fulfilled by CR-082 (ADD document
  type)'
---
...
| INV-011-CAPA-003 | Preventive | Implement ADD (Addendum) document type for post-closure corrections to executable documents. Spawn child CR. | Implemented via CR-082 (CLOSED v2.0). ADD document type added to qms-cli: creation requires CLOSED parent, supports CR/INV/VAR/ADD parents, follows full executable lifecycle. 7 new qualification tests, 403 total tests pass at commit dffd56c. TEMPLATE-ADD v1.0 EFFECTIVE. RS v13.0 and RTM v15.0 EFFECTIVE. CLI-7.0 qualified baseline. | Pass | claude - 2026-02-15 |
...
| **UAT Results (Session 2026-02-10-005):** Verified CAPA-001 (single-authority HTTP transport) with 7 tests, all PASS. P1-T1: Host trusted mode via HTTP (PASS). P1-T2: Container enforced mode via header, `qa` registered as instance `1647e18c` (PASS). P1-T4: HTTP without header handled as explicit trusted mode, no old-style warning (PASS — was VULNERABILITY). P2-T1: Container locks identity (PASS). P2-T2: Trusted-mode `user=qa` rejected with IDENTITY LOCKED while QA container holds lock (PASS — was NOT TESTABLE). P2-T3: Duplicate container blocked by agent-hub (PASS). P2-T4: `qa` and `tu_ui` coexist without collision (PASS). | claude - 2026-02-10 |
| **UAT Observation (P1-T3):** Identity mismatch not visible to caller. When tu_ui container passed `user="tu_sim"`, server correctly overrode to `tu_ui` (enforced mode) and logged `WARNING - Identity mismatch: header=tu_ui, param=tu_sim`. However, the container agent received no indication that its `user` parameter was ignored — it believed it queried tu_sim's inbox. The mismatch warning exists only in the server log. Flagged for future investigation: callers should be informed when identity override occurs. Added to TO_DO_LIST.md. | claude - 2026-02-10 |
| CAPA-003 fulfilled by CR-082 (CLOSED 2026-02-15). ADD document type implemented with full executable lifecycle, CLOSED-parent enforcement, and 7 qualification tests. All 3 CAPAs now Pass. | claude - 2026-02-15 |
...
All 3 CAPAs completed successfully.

| CAPA | Type | Result |
|------|------|--------|
| CAPA-001 | Corrective | `.mcp.json` updated to HTTP transport. Verified via UAT (7/7 tests pass). |
| CAPA-002 | Preventive | QA post-review prompt strengthened with explicit EI cross-reference check. |
| CAPA-003 | Preventive | ADD document type implemented (CR-082). Provides lightweight post-closure correction mechanism, eliminating the need for full INV/CAPA cycles for minor omissions. |

Root causes addressed:
1. **QA review gap (primary):** CAPA-002 ensures QA cross-references each EI's execution summary against its task description.
2. **No post-closure correction mechanism (secondary):** CAPA-003 provides the ADD document type for post-closure corrections to executable documents.

The original deviation (CR-075 incomplete EI-3) was remediated by CAPA-001. No further deviations were discovered during execution.
