...
revision_summary: Both CAPAs complete. CAPA-001 (CR-045, CLI-5.0). CAPA-002 (CR-046/CR-047,
...
| INV-008-CAPA-002 | Preventive | Containerization initiative operational: agents execute from containers with read-only access to production code | CR-046 executed and verified. Container Claude successfully called qms_inbox MCP tool via streamable-http transport. CR-047 added streamable-http support (SSE deprecated). Production QMS mounted read-only; MCP server on host handles all writes. | Pass | qa - 2026-02-02 |
...
| CAPA-002 complete: CR-046 executed. During testing, SSE transport found deprecated/buggy in Docker; CR-047 added streamable-http support (RS v6.0, RTM v7.0). Final test: container Claude called qms_inbox successfully. | claude - 2026-02-02 |

---

## 9. Execution Summary

Both CAPAs completed successfully.

**CAPA-001 (Corrective - Requalification):** CR-045 added SSE transport integration tests. CLI-5.0 qualified at commit 6212cc1. RTM EFFECTIVE v6.0.

**CAPA-002 (Preventive - Containerization):** CR-046 verified containerization operational. During testing, SSE transport was found to be deprecated and buggy in Docker environments (Claude Code GitHub issues #18557, #20335, #3033, #4202). CR-047 added streamable-http transport support (RS v6.0, RTM v7.0, CLI-6.0). Final verification: container Claude successfully called qms_inbox MCP tool via streamable-http transport.

The containerization architecture now provides the preventive control: agents operating from containers have read-only access to production code, with all QMS writes flowing through the MCP server on the host.
