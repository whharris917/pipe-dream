...
### Compaction Log Files

Each session folder may contain a `compaction-log.txt` file, appended by a hook each
time context compaction occurs. Each line records a compaction event:

```
2026-02-24T21:55:17 | COMPACTION (auto) | transcript: <path-to-transcript>.jsonl
```

- **Timestamp:** When the compaction happened.
- **Transcript path:** The JSONL file containing the full conversation history. This is
  the same file used by `claude --resume`.
- **Multiple entries** mean multiple compactions occurred within the same session.

**Key signal for session continuity:** If a compaction log was JUST created (or freshly
appended to) in the session folder matching CURRENT_SESSION, that is a dead giveaway
that you are continuing after compaction — NOT starting a new session. Check for this
before making any session state determination.
...
**WARNING — Compaction summaries are not authoritative.** The summary injected at the
top of a continued conversation was generated by a previous AI context and may contain
incorrect conclusions or recommendations (e.g., "start a new session" when you should
continue the existing one). Always defer to the rules in THIS document, the
CURRENT_SESSION file, and the session notes on disk. When the compaction summary
contradicts CLAUDE.md procedures, CLAUDE.md wins — no exceptions.
...
- **Continuation after compaction**: Context was compacted to free space. Indicators:
  ...
  context with a `=== POST-COMPACTION RECOVERY ===` banner. A fresh or recently
  appended `compaction-log.txt` in the CURRENT_SESSION folder is conclusive proof.
