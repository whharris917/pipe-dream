...
**Open work:** Most recent landings are workshop/theme-system improvements (workshop hub, right-click inspect panel, Navigation split, theme refactor + 6 themes, nesting-visualizations workshop). The strategic next step is to build real QMS workflow pages on top of Razem rather than continuing engine-side iteration. **65 CRs CLOSED, 5 INVs CLOSED.**
...
| **Nesting workshop follow-through.** Promote winners (Exp 4.1 inline disclosure, Exp 4.3 chat-aesthetic + completion) into a real engine surface; evaluate Exp 4.1.1 brackets variant; possibly retire Exp 4.5 if redundant. | Medium | Session-2026-04-21-001 |
