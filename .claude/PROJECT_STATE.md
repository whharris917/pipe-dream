...
*Last updated: 2026-05-01 (Session-2026-05-01-001 — CR-112 EIs 10-12 complete; merged + tagged FLOW-STATE-1.1; CLAUDE.md drift corrected)*
...
**Active focus:** CR-112 closing — EI-13 post-execution baseline + closure remaining.

**CR-112 (ToolContext Migration Completion + Documentation Reconciliation)** — IN_EXECUTION at v1.0. Bundled CR addressing the SourceTool ToolContext-migration miss (the user-visible crash from first post-CR-111 use) plus the three follow-ups CR-111 queued: Tool base-class `self.app` cleanup, `ctx.set_interaction_data` adoption, and CLAUDE.md §§2.3/6.2/7 drift. EIs 1-12 complete. Qualified commit flow-state@`ec450e2` reachable from main via merge commit `c82c8e2`. **FLOW-STATE-1.1** annotated tag pushed to flow-state remote at the merge. pipe-dream submodule pointer advanced to `c82c8e2` in pipe-dream@`9502287`. CLAUDE.md drift corrected: §2.2 (Compiler now described as two-way), §2.3 (interaction_data ownership clarified — on Sketch, with facade-routing requirement), §4.2 (interaction_data exception to direct-modification rule), §5.3 (Servo injection routes through facade), §5.4 (tethered-atom distinction added; two-way coupling), §5.5 (no `self.app` passthrough; expanded facade surface; underscore-prefixed `_get_sketch`/`_get_scene` for constrained subclass-property use), §6.2 (input layer order fixed: System → Modal → Global → HUD), §7 (orchestration loop rewritten as 8-step including topology check, physics sub-loop, post-constraint sync, process objects). EI-13 remaining: post-execution baseline commit, route post-review, approval, close.
...
| **SDLC-FLOW-RTM** | **v2.0 EFFECTIVE** | **CR-112 EI-10 — qualified at flow-state@`ec450e2`; System Release FLOW-STATE-1.1; cycle 1 QA found 5 line-citation drift findings; cycle 2 QA recommend; cycle 3 procedural reset (TU re-review escalation); cycle 4 qa+tu_ui+tu_sketch+tu_scene clean recommend → approved** |
| SDLC-WFE-RS | v0.1 DRAFT | Razem track; rewrite deferred while Flow State is active |
| **Qualified Baselines** | **CLI-18.0 + FLOW-STATE-1.1** | qms-cli main at `309f217`; flow-state main at `c82c8e2` (merge of cr-112-toolcontext-completion; qualified at `ec450e2`); annotated tag `FLOW-STATE-1.1` |
...
|----------|--------|---------|
| **CR-112** | **IN_EXECUTION v1.0** | **ToolContext Migration Completion + Documentation Reconciliation. EIs 1-12 Pass; qualified commit flow-state@`ec450e2` merged at `c82c8e2`; FLOW-STATE-1.1 tagged. EI-13 (post-execution baseline + closure) remaining.** |
...
1. **Close CR-112.** EI-13 only — post-execution baseline commit, post-review cycle, closure approval.
...
4. **Tool-facade architectural follow-up CR** (new from CR-112 cycle 4 TU reviews): both tu_ui and tu_sketch independently flagged that the leading-underscore convention on `ctx._get_sketch()` / `ctx._get_scene()` (currently used as the sanctioned subclass-property delegation surface) maintains the Air Gap by *discipline* rather than by *structure*. Two candidate paths: (a) rename to first-class facade methods (matching the WARNING-on-public-name pattern of `get_entity_direct`); (b) introduce a typed read-only `SketchView` protocol with structural enforcement; (c) eliminate the Tool subclass `@property sketch`/`@property scene` accessors entirely. Non-blocker for v1.1 — explicitly captured as backlog candidate by both TUs. Decide approach when it bites.
5. **Auto-mode-vs-subagent permissions resolution.** Either add a more specific allow-rule to settings.local.json (e.g., `Bash(python qms-cli/qms.py --user qa *)`), or migrate QA approval flows to MCP tools (`mcp__qms__qms_approve` is already allow-listed) to bypass the Bash permission layer. Did not bite during CR-112 EI-10 cycles.
...
6. **MCP-call primitive (Action variant for MCP tool invocation).** Was "terminal-execution primitive"; reframed. New Action variant invokes an MCP tool with form-validated arguments, binds the structured response into store keys consumable by other components. Engine speaks MCP; page seeds curate which MCP tools are exposed. This is the load-bearing piece — without it, every Razem workflow page is a stub. Cleaner than subprocess-shelling because it's protocol-typed end-to-end.
7. **Continue building real QMS workflow pages (= page seeds for the QMS MCP server).** After the MCP primitive: CR review, document lifecycle, inbox actions, route-for-review/approval. cr-create is the existing scaffold; once the MCP primitive lands, its Submit goes from stub to functional. Per the framing: page seeds ARE the procedural-and-HATEOAS layer that MCP omits — they encode the workflow between MCP boundary calls.
8. **Wire QMS / Workspace / Inbox to the QMS MCP server** rather than placeholder lists.
9. **Triage demo pages.** Decide which seeds in `/portal` are valuable vs gallery cruft.
10. **Framework rename: `qms-workflow-engine` → Razem.** Decided 2026-04-17-003. Scope: GitHub repo rename + remote URL update; `.gitmodules` and submodule path; `CLAUDE.md` references; Framing page; README; Lessons; Deep Dive intro. Effort: one focused session, low technical risk.
11. **Retire `qms_interact` from MCP surface** once Razem-on-MCP renders the equivalent procedural flows. `qms_interact` is the primordial idea that led to Razem; once Razem subsumes it, MCP can return to being purely about boundary actions.
12. **Long-term migration (multiple sessions): documentation-in-the-interface.** Procedural Quality Manual content moves into Razem page seeds (form labels, help text, validation, conditional visibility); policy content stays as a meta-document. CLAUDE.md shrinks to ~200-500 tokens. Session notes eventually move into the Portal. SELF.md may stay as a file because it's identity-not-procedure.
