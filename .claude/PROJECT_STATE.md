...
- **Affordance flotation (Phase 1)**: Separable affordances (Clear, Edit, Batch) float from child eigenforms to PageForm level during agent-facing serialization. Affordances tagged with `_floatable` merge key in `_serialize_full()`. PageForm collects, groups by key, and emits parameterized compound affordances with structured `targets` dict (key→label). Children retain only type-specific affordances. HTML rendering unaffected (flotation is agent-tier only). ~61% reduction in affordance block count on typical pages.
...
**Affordance Flotation — Phase 1** (Apr 4, session 003). Separable affordances (Clear, Edit, Batch) float from child eigenforms to PageForm during agent-facing serialization. Affordances tagged with `_floatable` merge key in base `_serialize_full()`. PageForm collects from children via `_serialize_full()` (preserving tags), groups by merge key, strips from children, emits parameterized compound affordances with structured `targets` dict (key→label) and generic instructions. HTML rendering unaffected — flotation is agent-tier only. ~61% reduction in affordance blocks (49→19 on Employee Onboarding page). No changes to routing, parity tests, or affordance dataclass. Research confirmed this pattern is novel — no existing hypermedia format has affordance flotation/merging. Closest precedents: Hydra class-level `supportedOperation`, MCP community tool consolidation.
...
- **Affordance flotation Phase 2+** — recursive flotation through nested containers, body-varying merges, container-level flotation, dynamic flotation rules
