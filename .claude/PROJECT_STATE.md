...
- **Sleek theme**: Dark UI inspired by VS Code / GitHub Dark Dimmed. `sleek.css` with navy palette (#1e2a3a page, #363b42 cards, #22272e titlebar, #2b3035 tab content). Blue accent (#2563eb/#3b82f6/#60a5fa), red focus (#ff6b6b), amber chrome (#d29922). Card-based data eigenforms with black borders; containers transparent. Sleek wrapper: data forms get card+titlebar, containers transparent+chrome only, GroupForm no titlebar (sidebar handles it), NavigationForm full titlebar with label+instruction+chrome. Vertical tabs: top-level NavigationForm tab bar in left margin with VS Code design language, titlebar extends left to align. Nested navs: horizontal tabs, normal titlebar. GroupForm sidebar: label/instruction/affordances in right margin (`sleek/group.html`). Page padding 12% left/right. PageForm separator below header.
