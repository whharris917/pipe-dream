...
**Affordance Framework Redesign** (Mar 22). Affordances anchored to the objects they operate on (fields, table, columns, properties). Field-affordance unified response. Renderer cleanup: merged into single `human-renderer.js`, renamed `raw.js` → `agent-renderer.js`.

**Workshop Experimentation** (Mar 22). Built four workshop pages: API Design (JSON↔HTML cross-highlighting), Element Library (registry-based self-rendering elements, container-based focus, sparse focus map, agent data projection), Workflow Console (full-stack demo with 13 element types, auto-focus, step gating, computed values). Three-agent LNARF audit of the console found 7 lossless violations — all fixed. Live demo: created a CR via the agent API using only affordance-driven POSTs.

**Focus Mechanism Excision** (Mar 22). Designed hierarchical focus/attention mechanism in workshop, then determined it was over-engineering: well-designed workflow nodes should be small enough that agents see everything. Excised focus from the real engine: removed `_focus`/`_unfocus` actions, focus-based affordance filtering in renderer.py (~130→~30 lines), focus zone rendering in human-renderer.js. All affordances now always visible — no focus/unfocus cycle required. Fixed duplicate field affordance rendering after excision.
...
**Content Array + Element Type Vocabulary + Element Library** (Mar 23). Unified `fields`/`field_groups` state keys into a single ordered `content` array — element type embedded in each item, rendering order follows YAML definition order exactly. Renamed field types from Python-isms (`boolean`, `select`, `computed`) to semantic names (`toggle`, `choice`, `indicator`) with backward-compat normalization maps; all 11 YAML workflow files updated. Runtime instruction enforcement: `FieldGroupDef` and `NodeDef` raise `ValueError` at load time if `instruction` is missing. Combined all Change Definition fields into a single group. Rebuilt Element Library workshop page — grounded in actual engine schema and renderer. Interactive demos for all 7 element types (rendered via actual `human-renderer.js` functions, affordance calls intercepted client-side). Added 5 new sections: `visible_when`, `side_effects`, `proceed`, `navigation`, `expressions` — covering behavioral features, flow control, and the expression language. "Agent view" panel shows both content and affordances together, updating live as state changes. Tabbed UI with group labels in the tab bar. Deleted three stale workshop pages (API Design, Element Library, Workflow Console).
...
| `app/templates/workshop_elements.html` | Element Library — interactive reference for all 7 element types + behavioral features (visible_when, side_effects, proceed, navigation, expressions). Tabbed UI, live Agent view panel. |
