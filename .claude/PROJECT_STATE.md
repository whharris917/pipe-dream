...
*Last updated: 2026-04-29 (Session-2026-04-29-001)*
...
**Open work:** Session-2026-04-29-001 converged Experiment D onto a **unified Tabs-vs-Sequence visual motif** through six successive refinements: (1) T-junction motif for parallel tabs (replacing the JS-driven shift-to-thread), (2) drop tab-level gating so Tech/Bus/QA become genuinely parallel (only the outer Phase 1 → Phase 2 sequence is gated), (3) drop the redundant "Phase X of Y" / "Step X of Y" framing from labels (the gated UI already conveys it), (4) unify all gated sequences under a single S-junction motif with 🔒-prefixed pills (replacing the prior numbered chat-bubble avatars), (5) make sequence pills vertical with the active step's content interleaved between sibling rows ("content sits between this step's button and the next"), and finally (6) shift the sequence pills onto the main thread itself (drop snode + connector — pill sits flush at x=0 so the thread intersects it top-to-bottom). The result is a clean rule: **Tabs (parallel) → horizontal T-junction; Sequence (gated) → vertical pills that intersect the thread, with content between siblings**. Prior sessions' Experiment D pages (numbered chat-bubble per phase, separate ad-hoc rendering for QA's inner sub-flow) are now retired; everything uses the unified motif at every nesting depth. The strategic next step remains building real QMS workflow pages on top of Razem; this motif is a strong candidate for the engine-side surface. **65 CRs CLOSED, 5 INVs CLOSED.**
