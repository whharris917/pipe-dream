...
*Last updated: 2026-04-27 (Session-2026-04-26-001)*
...
| **Graph Builder follow-through.** Implement compile-to-Razem (item 4): a recursive walk over the tree emits Razem JSON — `next` chains → `SequenceForm` steps; `branch` rows → `TabForm` / `Group` / `Switch` per the container's kind. The graph IS the Razem nesting; no structural inference needed. Once compile lands, Graph Builder becomes a real authoring surface for QMS workflow pages. Polish items: an Add Branch affordance more discoverable than the tag handle (e.g., a `+` ghost at the right edge of the rightmost branch); rename UI; persistence so refresh doesn't reset; nested-container layout collisions in pathological cases (current width-aware allocation handles common cases but isn't dominator-tree-aware). | Medium | Session-2026-04-26-001 |
