...
*Last updated: 2026-05-12 (Session-2026-05-12-002 — Demo presets reworked end-to-end: 4 PBC density-spectrum demos shipped (Sparse/Liquid/Dense/Packed); demo→UI sync fix so preset physics knobs survive the per-frame UI→sim read; existing 3 demos rewritten to fill the volume under PBC at proper densities; micelles preset rewritten with pre-organized layout after empirical diagnostic showed random-IC self-assembly is not feasible on demo timescales; quantitative phenomenology test added. Suite 941 → 968 passing + 1 skipped, +27 tests.)*
...
- **CR-116 EI-3 Demo Preset Rework (May 12, Session-2026-05-12-002).** Four-round arc reworking the Demos menu to actually deliver what its labels claim. **R1 (PBC density-spectrum demos):** new `preset_pbc_liquid(scene, density, target_temp, material, seed)` parameterised by reduced density ρ* = N σ² / L²; four canonical wrappers wired through menu/dispatch (PBC Sparse ρ*=0.2 / PBC Liquid ρ*=0.5 / PBC Dense ρ*=0.8 / PBC Packed ρ*=1.0 with T bumped to 1.2 to stay fluid); new `_grid_positions_pbc_fill` helper tiling `[0, L)²` uniformly under PBC (distinct from the existing centred-sub-tile `_grid_positions`). +15 tests. **R2 (universal demo→UI sync fix):** root cause — `flow_state_app.update` reads UI sliders/buttons into `sim` every MODE_SIM frame, so the preset's `sim.gravity = 0` etc. got clobbered the next frame; symmetric fix pushes sim → UI after every preset run. New `SmartSlider.set_val` public method (thin wrapper around `_apply_value_with_expansion` + input-field refresh) + new `AppController._sync_sim_controls_to_ui` (pushes gravity/temp/damping/dt/skin/thermostat/boundaries to widgets). Universal across all 7 preset branches. +7 tests. **R3 (existing demos PBC refactor):** shared `_seed_pbc_demo_state` helper consolidates gravity=0/damping=0.999/thermostat=on/PERIODIC; Demixing/Micelles/Crystal Anneal switched to `_grid_positions_pbc_fill` so the volume is uniformly filled; default counts bumped (Demixing 160→1200, Micelles 160→1100, Crystal Anneal 120→1600) and target_temps tuned to the 2D LJ phase diagram (Demixing T=0.7 above coexistence ceiling; Micelles T=0.6; Crystal Anneal T=1.5 unchanged). Dead `spacing` / `solvent_spacing` parameters removed. +2 tests (crystal_anneal PBC + gravity checks). **R4 (micelles empirical-driven rewrite):** Lead asked for theoretical and quantitative evidence that the micelles preset shows what it claims. Honest answer: had neither. Wrote a diagnostic (`_micelle_diagnostic.py`, deleted at end of round) that loads the preset, integrates for 50k-500k substeps, and reports cluster sizes + head-out / tail-in metrics. Empirical finding: random-IC self-assembly is NOT feasible on demo timescales — surfactant diffusive encounter takes ~10⁶ substeps at our concentration, vs ~10⁵ available in a demo. Parameter sweeps (2× concentration, T=0.3, ε_PN=0.05) didn't fix it. Resolution: rewrote `preset_micelles` with a deterministic pre-organised layout — 4 micelles on a 2×2 grid, K=6 surfactants per micelle on a ring of radius R=3.3σ with rotation θ+π so heads point outward and tails converge near the cluster centre. New helpers `_compute_micelle_layout` + `_surfactant_atom_positions`; solvent fill skips grid cells within 1σ of any planned surfactant atom to prevent initial-step LJ blow-up. Empirical re-verification across 3 seeds × 50k substeps: 4 distinct tail-tip clusters persist; sizes GROW from 6 → 9-13 as surfactants consolidate (no dissolution, no blob fusion); head-out structure robust (hs-NN ≈ 1.2σ vs ht-NN ≈ 3.5-3.8σ). +3 quantitative tests in `TestMicellesPhenomenology` covering t=0 initial state, post-relaxation cluster stability, and head-out / tail-in metric. Status text revised to honestly describe "pre-arranged proto-micelles stabilised by the hydrophobic effect" rather than the previous (unverified) self-assembly claim. Suite trajectory across the session: 941 → 956 (R1) → 963 (R2) → 965 (R3) → **968 passing + 1 skipped** (R4).

- **CR-116 EI-3 PBC Density-Gradient Fix (May 12, Session-2026-05-12-001).** Lead-reported physics bug: under PERIODIC boundaries large simulations developed visibly lower density near the boundaries. Root cause: `build_neighbor_list` used `n_cells = int(world_size // cell_size) + 1` — the `+1` is a padding cell that's correct for OPEN/REFLECTING (catches atoms briefly outside `[0, L)`) but wrong under PBC, where the wrap forces positions strictly inside `[0, L)`. Using the inflated `n_cells` as the wrap-arithmetic modulus made cell 0's `dx=-1` wrap land on cell `n_cells-1` — empty (exact `L % cs == 0`) or only partly populated (non-exact). Atoms within ~`cell_size` of either edge lost some/all of their cross-wrap LJ partners → persistent net inward force → steady-state low-density boundary layer. Fix branches `n_cells`/`inv_cell` on `boundary_mode`: under PBC, `n_cells = max(3, int(L // cs))` and `inv_cell = n_cells / L` so cells tile `[0, L)` exactly with effective width `L/n_cells` (≥ cs since floor); under OPEN/REFLECTING, formula unchanged. `_pbc_safe` tightened to match (`int(L//cs) >= 3`, formerly `>= 2` which was both permissive and buggy). 4 new tests in `TestPeriodicCellListBoundaryDepth`: exact-divisibility wrap (ws=28 = 10×cs), beyond-last-cell wrap at default ws (atom at x=0.1 paired with x=47.5 in cell 16 not searched pre-fix), diagonal-corner wrap at ws=28, and an interior 3-atom control. Suite **937 → 941 passing + 1 skipped**, zero regressions across full run.
...
- `flow-state/tests/` — **968 passing + 1 skipped** end of Session-2026-05-12-002.
...
|----------|--------|---------|
| **CR-116** | **IN_EXECUTION v1.2** | First Exploration CR. EI-1 + EI-2 Pass; EI-3 active. Branch `cr-116-beach-trip-exploration`. 968 passing + 1 skipped. |
