...
*Last updated: 2026-05-12 (Session-2026-05-12-001 — PBC density-gradient root-cause fix: cell-list n_cells off-by-one in `build_neighbor_list` mis-aimed wrap arithmetic at an empty/partial padding cell instead of the populated cross-boundary cell, depleting atoms within ~cell_size of either edge of their wrap-side LJ partners; fix branches n_cells/inv_cell on boundary_mode so PBC tiles [0, L) exactly. Suite 937 → 941 passing + 1 skipped, +4 tests.)*
