...
*Last updated: Session-2026-04-21-001 (2026-04-25)*

---

## 1. Where We Are Now

**New workshop: Nesting Visualizations + 11 experiments (Session-2026-04-21-001).** New entry in the workshop hub at `/workshop/nesting`, fully self-contained: single template `qms-workflow-engine/app/templates/workshop_nesting.html` (~2500 lines), no hooks into the engine or component system. Each experiment is an independent IIFE with its own scoped CSS+JS so iteration is friction-free. Workshop chrome borrows the chat theme aesthetic (pastel radial-gradient canvas, frosted-glass topbar with indigo→purple gradient on the active experiment chip).

The workshop is a design playground for the question "how should Razem visualize nested structure, and where do per-group instructions go?" Eleven experiments across two groups plus one late-session variant:

*Group 1 (tab-centric progression):* **Exp 1** naive nested tabs (3×3×3) with instruction as plain paragraph above each bar — shows the failure mode of stacked undifferentiated chrome at 3 levels deep. **Exp 2** moves the instruction into the tab bar itself as the first flex child; instruction and tabs share a baseline and bottom border (the minimum-viable tether). **Exp 2 (extended)** allows the bar to grow vertically with `align-items: flex-end` for multi-paragraph instructions, with tab buttons pinned to the bottom border. **Exp 3** moves instructions into a click-triggered popover anchored to a `?` button on each bar — frosted-glass card with tail arrow, escape/outside-click to dismiss, at most one open at a time. **Exp 4** is a VS Code–style split: tree explorer in a 280px dark sidebar (chevron + folder/file icon + indent guides), details pane on the right showing instruction (folder selected) or leaf content (leaf selected). First experiment where nesting-rendering and instruction-placement are fully decoupled. **Exp 5** keeps the Exp 2 tab tree but moves instructions into a Word/Google-Docs-style left margin column — comments are absolutely positioned via `getBoundingClientRect()` delta to vertically align with their associated tab bar; failure mode is overlap when comments are taller than inter-bar spacing. Lead's verdict: Exp 4 is the Group 1 winner.

*Group 2 (variations originating from Exp 4):* **Exp 4.1** dissolves the right pane — content expands inline below each node, indented with chevron. **Exp 4.2** is Miller columns (Finder-style horizontal drill-down): each depth gets its own 260px column, each column has a header with column-tag and instruction, leaf selection appends a detail column. Trades horizontal real-estate for "all sibling sets visible at once." **Exp 4.3** is Exp 4 re-skinned in the chat aesthetic (frosted-glass panes, indigo→purple gradient on selected pill rows, purple/pink CSS-drawn icons), then enhanced with completion/progress mechanics: leaf checkboxes (gradient-fill when done), folder N/M counter pills (gradient when 100%), strikethrough on done leaves, global N/M counter in tree header, gradient progress bar in details pane for folders, "Mark as done"/"✓ Completed" toggle pill for leaves. **Exp 4.4 (replacement)** is collapsed Miller — the current scope occupies the whole canvas; ancestors collapse into a clickable breadcrumb (`Top › Tab 1 › Tab 1.2`) plus an Up button. Scalable variant of 4.2. The original Exp 4.4 (multi-pin compare with shift-click pinning) was discontinued as "too task-management-suite." **Exp 4.5** is the VS Code-aesthetic counterpart to 4.3's chat-aesthetic: same completion mechanics, dark sidebar styling, retained as an aesthetic comparison point. **Exp 4.1.1** is a late-session variant of 4.1 that addresses implicit-grouping by wrapping each (instruction + siblings) group in a `.exp411-group` container with a 2px solid indigo line in its left gutter; opacity fades with depth (0.45 → 0.32 → 0.22) so stacked brackets at three levels don't pile into visual noise.

**Lead's verdicts after Group 2:** Exp 4.1 and Exp 4.3 (with 4.5-merged completion) are the winners. Exp 4.5 retained for aesthetic comparison; user may discontinue if redundant. Exp 4.1.1 awaiting evaluation.
