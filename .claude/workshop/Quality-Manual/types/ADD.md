...
**Key points:**
- ADDs require a `--parent` flag and `--title`
- Parent type must be CR, INV, VAR, or ADD
- **Parent must be CLOSED** -- this is the defining characteristic of ADD
...
ADDs follow the standard executable document workflow. See [CR Reference](CR.md) for the full state machine. Note that when an ADD closes, cascade close applies to child attachment documents (VR type).
