...
The template defines a structure for investigating deviations. See [Deviation Management](../05-Deviation-Management.md) for when and how to investigate.
...
| 2.2 | Deviation Type | Procedural or Product |
...
INVs follow the same executable workflow as [CRs](CR.md). All status transitions, checkout behavior, and approval gates are identical. VR documents cannot have INV parents (only CR, VAR, or ADD), so cascade close for VR attachments does not apply to INVs.
...
Note: ADD documents **can** be created against closed INVs.
...
| VR support | Yes (VR can have CR parent) | No (VR requires CR, VAR, or ADD parent) |
