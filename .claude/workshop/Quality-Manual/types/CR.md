...
The template defines a strict structure. See [Change Control](../04-Change-Control.md) for content requirements.
...
- **Checkout from PRE_APPROVED** triggers a scope revision (returns to DRAFT)
- **Checkout from IN_EXECUTION** increments the minor version
- **Approval gate** requires all reviewers to recommend and at least one quality-group reviewer
- **Close** cascades to child attachment documents (VR type)
- **Cancel** is only available for version < 1.0
