...
Every lifecycle event is recorded in `.audit/` JSONL files. These are append-only -- events are never modified or deleted. The audit trail is the ground truth for what happened, when, and by whom. See the [CLI Reference](../qms-cli/docs/cli-reference.md) for the `history` and `comments` commands that read these trails.
