...
- "LJ": Classical truncated Lennard-Jones 12-6 force
- "TAB_LJ": Tabulated LJ with linear interpolation in r^2 (no sqrt)
- "QUAD": Piecewise quadratic approximation using r^2

Switch at runtime: 1->LJ, 2->TAB_LJ r^2, 3->QUAD

Verlet pair-list mode:
- VERLET_MODE_ENABLED selects starting mode; toggle with V
- Pair list uses r_list = r_cut + r_skin; rebuilds when max displacement since last build exceeds r_skin/2

Parallel mode (pair-list only):
- PARALLEL_ENABLED selects starting parallel mode; toggle with P
- Uses Numba parallel=True with prange over pairs and thread-private force buffers, then reduces

Benchmark mode:
- Press b to benchmark current configuration for BENCH_SECONDS (no drawing/events)
- Press B (Shift+b) to benchmark a sweep of configurations (serial/parallel; chosen models)
- Results printed to console; brief results shown on-screen after the run
"""

FORCE_MODEL = "LJ"
VERLET_MODE_ENABLED = True   # toggle with V at runtime
PARALLEL_ENABLED = True      # toggle with P at runtime

# ============================================================
# Benchmark settings
# ============================================================
BENCH_SECONDS = 5.0
BENCH_SWEEP_MODELS = ["LJ", "TAB_LJ", "QUAD"]
BENCH_SWEEP_PARALLEL = [False, True]
BENCH_SWEEP_RPT = 1
...
L = np.float32(math.sqrt(N / float(density)))  # box size
...
draw_every_M = 10  # also used as nsteps per fused kernel call
show_extra_diag = True  # toggle with H

# Neighbor list / cell list parameters (cell mode fallback)
...
# ============================================================
# Pair-list building and displacement tracking (SoA)
# ============================================================
@njit(fastmath=True)
def compute_max_displacement_soa(pos_x, pos_y, last_x, last_y, L):
    Nloc = pos_x.shape[0]
...
# ============================================================
# Serial fused multi-step (pair-list) kernel
# ============================================================
@njit(fastmath=True)
def step_pairs_n_serial(
    nsteps,
        ...
        for idx in range(pair_count):
            ...
            if r2 < r_cut2 and r2 > np.float32(1e-12):
                if model_id == 0:
                    ...
                    f_over_r = force_QUAD_from_r2(r2, sigma*sigma, r_a2, r_cut2_q, k_rep, k_mid, k_tail)
                fx = f_over_r * dx
                fy = f_over_r * dy
                force_x[i] += fx
                force_y[i] += fy
...
# ============================================================
# Parallel pair force step (1 step), then loop it for nsteps
# ============================================================
@njit(parallel=True, fastmath=True)
def pair_forces_parallel(
    pos_x, pos_y,
    ...
    for t in prange(T):
        for i in range(Nloc):
            fx_buf[t, i] = np.float32(0.0)
            fy_buf[t, i] = np.float32(0.0)
    # pair loop
    for idx in prange(pair_count):
        i = pair_i[idx]
        j = pair_j[idx]
        dx = pos_x[i] - pos_x[j]
        dy = pos_y[i] - pos_y[j]
...
@njit(fastmath=True)
def reduce_force_buffers(fx_buf, fy_buf, force_x, force_y):
    T = fx_buf.shape[0]
    Nloc = force_x.shape[0]
    for i in range(Nloc):
        sx = np.float32(0.0)
        sy = np.float32(0.0)
        for t in range(T):
...
@njit(fastmath=True)
def integrate_half_positions(pos_x, pos_y, vel_x, vel_y, force_x, force_y, L, dt, mass):
    half_dt = np.float32(0.5) * dt
    dt2_over2m = np.float32(0.5) * dt * dt / mass
    Nloc = pos_x.shape[0]
    for i in range(Nloc):
...
@njit(fastmath=True)
def finish_velocity(vel_x, vel_y, force_x, force_y, dt, mass):
    half_dt = np.float32(0.5) * dt
    Nloc = vel_x.shape[0]
    for i in range(Nloc):
        vel_x[i] += (force_x[i] / mass) * half_dt
...
def ensure_buf_shape(buf, T, N):
    if buf is None or buf.shape != (T, N):
        return np.zeros((T, N), dtype=np.float32)
    return buf

# ============================================================
# Cell-mode (fallback) compute
# ============================================================
@njit(fastmath=True)
...
def main():
    ...
    global thermostat, FORCE_MODEL, VERLET_MODE_ENABLED, PARALLEL_ENABLED
    global dt, density, L, scale, r_skin, r_list, r_list2
    global ncell_x, ncell_y, cell_size

    pygame.init()
    screen = pygame.display.set_mode((WINDOW_W, WINDOW_H))
    pygame.display.set_caption("2D MD - Numba float32, SoA, parallel pairs, benchmarks (fixed)")
    ...
    max_pairs = 40 * N  # generous cap; increase if needed
    pair_i = np.zeros(max_pairs, dtype=np.int32)
    pair_j = np.zeros(max_pairs, dtype=np.int32)
    pair_count = 0

    # Track positions at last pair build
    last_x = pos_x.copy()
    last_y = pos_y.copy()

    # Thread-private force buffers (allocate to current thread count)
    T_threads = max(1, get_num_threads())
    fx_buf = np.zeros((T_threads, N), dtype=np.float32)
    fy_buf = np.zeros((T_threads, N), dtype=np.float32)

    model_id = model_name_to_id(FORCE_MODEL)

    # Initial build and warm-up
    if VERLET_MODE_ENABLED:
        pair_count = build_verlet_pairs_soa(pos_x, pos_y, L, r_list2, cell_size, ncell_x, ncell_y, pair_i, pair_j)
        last_x[:] = pos_x
        last_y[:] = pos_y
        step_pairs_n_serial(
            1, pos_x, pos_y, vel_x, vel_y, force_x, force_y,
            pair_i, pair_j, pair_count,
            L, dt, mass,
            model_id, sigma, epsilon, r_cut2,
        ...
        build_cell_linked_list_soa(pos_x, pos_y, cell_heads, next_idx, L, cell_size, ncell_x, ncell_y)
        integrate_half_positions(pos_x, pos_y, vel_x, vel_y, force_x, force_y, L, dt, mass)
        ...
        finish_velocity(vel_x, vel_y, force_x, force_y, dt, mass)
    ...
    def do_benchmark(current_only=False):
        nonlocal bench_message
        # Save current config snapshot (for message only)
        saved = {
            "FORCE_MODEL": FORCE_MODEL,
            "model_id": model_id,
            "verlet": VERLET_MODE_ENABLED,
        ...
        def run_one(model_name, parallel_flag):
            local_model = model_name_to_id(model_name)
            # Build fresh pairs
            ...
            if parallel_flag:
                integrate_half_positions(pos_x, pos_y, vel_x, vel_y, force_x, force_y, L, dt, mass)
                pair_forces_parallel(
                    pos_x, pos_y,
                    pair_i, pair_j, pc, L,
                    local_model, sigma, epsilon, r_cut2,
                    tab_f_over_r, tab_r2_min, tab_r2_max, tab_dr2,
                    sigma2_q, r_a2, r_cut2_q, k_rep, k_mid, k_tail,
                    fx_buf, fy_buf
                )
                reduce_force_buffers(fx_buf, fy_buf, force_x, force_y)
                finish_velocity(vel_x, vel_y, force_x, force_y, dt, mass)
            else:
                step_pairs_n_serial(
                    1, pos_x, pos_y, vel_x, vel_y, force_x, force_y,
            ...
            t0 = time.time()
            steps = 0
            while (time.time() - t0) < BENCH_SECONDS:
                max_disp2 = compute_max_displacement_soa(pos_x, pos_y, last_x, last_y, L)
                threshold2 = (np.float32(0.5) * r_skin) * (np.float32(0.5) * r_skin)
                if max_disp2 >= threshold2 or pc == 0:
                    pc = build_verlet_pairs_soa(pos_x, pos_y, L, r_list2, cell_size, ncell_x, ncell_y, pair_i, pair_j)
                    last_x[:] = pos_x
                    last_y[:] = pos_y
                if parallel_flag:
        ...
        results = []
        if current_only:
            results.append(("current", FORCE_MODEL, PARALLEL_ENABLED, run_one(FORCE_MODEL, PARALLEL_ENABLED)))
        else:
            for rp in range(BENCH_SWEEP_RPT):
                for model in BENCH_SWEEP_MODELS:
                    for par in BENCH_SWEEP_PARALLEL:
                        sps = run_one(model, par)
                        results.append((f"run{rp+1}", model, par, sps))

        print("Benchmark results (steps/sec):")
        for tag, model, par, sps in results:
            print(f"{tag:6s} | model={model:6s} | parallel={'ON ' if par else 'OFF'} | {sps:10.1f}")
        if current_only:
            bench_message = f"Benchmark current: model={FORCE_MODEL}, parallel={'ON' if PARALLEL_ENABLED else 'OFF'}: {results[0][3]:.0f} steps/s"
        else:
            best = max(results, key=lambda r: r[3])
            bench_message = f"Benchmark sweep: best {best[1]} parallel={'ON' if best[2] else 'OFF'} at {best[3]:.0f} steps/s (see console)"
    ...
    while True:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_SPACE:
                    ...
                    draw_every_M = min(5000, draw_every_M + 1)
                    ...
                    r_skin = r_skin + np.float32(0.05) * sigma
                elif event.key == pygame.K_f:
                    r_skin = max(np.float32(0.0), r_skin - np.float32(0.05) * sigma)
                elif event.key == pygame.K_t:
                    dt = dt * np.float32(1.1)
                elif event.key == pygame.K_g:
                    dt = dt * np.float32(0.9)
                elif event.key == pygame.K_b:      # benchmark current config
                    do_benchmark(current_only=True)
                elif event.key == pygame.K_b and (pygame.key.get_mods() & pygame.KMOD_SHIFT):  # Shift+B sweep
                    do_benchmark(current_only=False)

        # Update derived settings after any key changes
        model_id = model_name_to_id(FORCE_MODEL)
        r_list = r_cut + r_skin
        r_list2 = r_list * r_list

        # Choose number of steps per cycle
        ...
        if VERLET_MODE_ENABLED:
            ...
            if PARALLEL_ENABLED:
                # Run nsteps one-by-one with parallel forces
                for _ in range(nsteps):
                    integrate_half_positions(pos_x, pos_y, vel_x, vel_y, force_x, force_y, L, dt, mass)
                    # Ensure buffers sized to current threads
                    T_threads = max(1, get_num_threads())
                    if fx_buf.shape[0] != T_threads:
                        fx_buf = ensure_buf_shape(None, T_threads, N)
                        fy_buf = ensure_buf_shape(None, T_threads, N)
                    pair_forces_parallel(
                        pos_x, pos_y,
                        pair_i, pair_j, pair_count, L,
                        model_id, sigma, epsilon, r_cut2,
                ...
                # Serial fused multi-step
                step_pairs_n_serial(
                    nsteps, pos_x, pos_y, vel_x, vel_y, force_x, force_y,
                    pair_i, pair_j, pair_count,
                    L, dt, mass,
                    model_id, sigma, epsilon, r_cut2,
                    tab_f_over_r, tab_r2_min, tab_r2_max, tab_dr2,
            ...
            # Cell traversal mode: 1 step per loop
            integrate_half_positions(pos_x, pos_y, vel_x, vel_y, force_x, force_y, L, dt, mass)
        ...
        # Render
        do_draw = True  # draw each loop after we advance by nsteps
        if do_draw:
            ...
            draw_text(screen, font, f"Force model: {FORCE_MODEL} (1=LJ, 2=TAB_LJ r2, 3=QUAD)", 20, y); y += 20
            draw_text(screen, font, f"Verlet pairs: {'ON' if VERLET_MODE_ENABLED else 'OFF'} (V)", 20, y); y += 20
            draw_text(screen, font, f"Parallel pairs: {'ON' if PARALLEL_ENABLED else 'OFF'} (P)", 20, y); y += 20
            draw_text(screen, font, f"pair_count: {pair_count}", 20, y); y += 20
            draw_text(screen, font, f"r_skin: {float(r_skin):.3f} (R/F +/-0.05*sigma)", 20, y); y += 20
            draw_text(screen, font, f"r_list: {float(r_list):.3f} (r_cut {float(r_cut):.2f})", 20, y); y += 20
            draw_text(screen, font, f"Draw every M: {draw_every_M} (D toggle, Up/Down adjust)", 20, y); y += 20
            draw_text(screen, font, f"dt: {float(dt):.5f} (T/G +/-10%)", 20, y); y += 20
            draw_text(screen, font, f"Threads (Numba): {get_num_threads()}", 20, y); y += 20
            if bench_message:
                y += 10
                draw_text(screen, font, bench_message, 20, y); y += 20
