...
def main():
    ...
    pygame.display.set_caption("2D MD - Numba float32, SoA, fused pair steps, diagnostics & runtime controls")
    ...
    while True:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_SPACE:
                    ...
                    # reset rebuild flags to switch modes cleanly
                    last_rebuild = -rebuild_interval
                    last_x[:] = pos_x
                    last_y[:] = pos_y
                    pair_count = 0
                elif event.key == pygame.K_r:  # increase r_skin
                    ...
                    last_x[:] = pos_x
                    last_y[:] = pos_y
                elif event.key == pygame.K_f:  # decrease r_skin
                    ...
                    last_x[:] = pos_x
                    last_y[:] = pos_y
                elif event.key == pygame.K_t:  # increase dt by 10%
                    dt = dt * np.float32(1.1)
                elif event.key == pygame.K_g:  # decrease dt by 10%
                    dt = dt * np.float32(0.9)
                elif event.key == pygame.K_PLUS or event.key == pygame.K_EQUALS:  # increase density by 5%
                    density = density * np.float32(1.05)
                    # recompute L and mappings
                    L = np.float32(math.sqrt(N / float(density)))
                    scale = (WINDOW_W - 2 * MARGIN) / float(L)
                    # update cell grid
                    ncell_x = max(1, int(float(L) // float(cell_size)))
                    ncell_y = max(1, int(float(L) // float(cell_size)))
                    # wrap positions into new box
                    for i in range(N):
                    ...
                    last_x[:] = pos_x
                    last_y[:] = pos_y
                elif event.key == pygame.K_MINUS:  # decrease density by 5%
                    ...
                    last_x[:] = pos_x
                    last_y[:] = pos_y
                elif event.key == pygame.K_i:  # reinitialize lattice positions
                    pos_x[:], pos_y[:] = init_lattice_positions_soa(N, L)
                    vel_x[:], vel_y[:] = init_random_velocities_soa(N, target_temp, mass)
                    pair_count = build_verlet_pairs_soa(pos_x, pos_y, L, r_list2, cell_size, ncell_x, ncell_y, pair_i, pair_j)
                    last_x[:] = pos_x
        ...
        if VERLET_MODE_ENABLED:
            ...
            # Thermostat during initial steps
