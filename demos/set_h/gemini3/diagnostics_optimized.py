...
- "TAB_LJ": Tabulated LJ with linear interpolation in r^2 (no sqrt in inner loop)
...
# NEW: Spatial sorting interval (Optimization)
sort_interval = 100
...
def init_lattice_positions_soa(N, L):
    ...
    for i in range(n_per_side):
        for j in range(n_per_side):
            if count >= N: break
            # Centered in the grid cell to avoid immediate boundary overlap
...
# ============================================================
# NEW: Spatial Sorting Helper (Optimization)
# ============================================================
@njit(fastmath=True)
def spatial_sort_soa(pos_x, pos_y, vel_x, vel_y, force_x, force_y, L, cell_size):
    ...
    # In-place reorder using temp copies
    pos_x[:] = pos_x[perm]
    pos_y[:] = pos_y[perm]
    vel_x[:] = vel_x[perm]
    vel_y[:] = vel_y[perm]
                    ...
                    for dy_cell in (-1, 0, 1):
                        ...
                        while jj != -1:
                            if jj > ii:
                                ...
                                r2 = dx * dx + dy * dy
                                # FIX: Added zero-division safeguard
...
# ============================================================
# Verlet pair-list building (FIXED)
# ============================================================
@njit(fastmath=True)
...
def build_verlet_pairs_soa(pos_x, pos_y, L, r_list2, cell_size, ncell_x, ncell_y, pair_i, pair_j):
    ...
    count = 0
    # FIX: Added limit to prevent memory corruption
                    ...
                    for dy_cell in (-1, 0, 1):
                        ...
                        while jj != -1:
                            if jj > ii:
                                ...
                                r2 = dx * dx + dy * dy
                                if r2 < r_list2:
                                    # FIX: Check bounds before writing
                                    if count < max_pairs:
                                        pair_i[count] = ii
                                        pair_j[count] = jj
                                        count += 1
...
# ============================================================
# NEW: Check skin breach inside fused loop
# ============================================================
@njit(fastmath=True)
...
def step_pairs_n(
    ...
    for s in range(nsteps):
        # NEW: Safety check every 5 steps
        if s > 0 and s % 5 == 0:
...
# ============================================================
# Pygame visualization helpers (RESTORED)
# ============================================================
...
def main():
    ...
    # Pair list buffers - Start slightly larger to be safe
    max_pairs = 100 * N 
    ...
    while True:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_SPACE:
                    ...
                    draw_every_M = min(5000, draw_every_M + 10)
                elif event.key == pygame.K_DOWN:
                    draw_every_M = max(1, draw_every_M - 10)
                ...
                elif event.key == pygame.K_g: dt = dt * np.float32(0.9)
                    ...
                    ncell_y = max(1, int(float(L) // float(cell_size)))
                    for i in range(N):
                        pos_x[i] = np.float32(pos_x[i] % L)
                        pos_y[i] = np.float32(pos_y[i] % L)
                    pair_count = build_verlet_pairs_soa(pos_x, pos_y, L, r_list2, cell_size, ncell_x, ncell_y, pair_i, pair_j)
                    last_x[:] = pos_x
                    last_y[:] = pos_y
        ...
        if VERLET_MODE_ENABLED:
            ...
            # FIX: step_pairs_n now handles skin check internally
            done = step_pairs_n(
                nsteps,
                pos_x, pos_y, vel_x, vel_y, force_x, force_y,
                pair_i, pair_j, pair_count,
            ...
            # If fused loop aborted early (due to skin breach), next loop triggers rebuild naturally

            if thermostat and step < thermostat_steps:
                apply_thermostat_numba_soa(vel_x, vel_y, target_temp, mass, mix=damping)
        else:
            # Cell traversal mode (Fallback)
        ...
        if draw_skip_enabled or not VERLET_MODE_ENABLED:
            ...
            draw_text(screen, font, f"Verlet pairs: {'ON' if VERLET_MODE_ENABLED else 'OFF'} (V toggle)", 20, y); y += 20
            if VERLET_MODE_ENABLED:
                ...
                threshold2 = (np.float32(0.5) * r_skin) ** 2
                draw_text(screen, font, f"rebuild threshold (disp^2): {float(threshold2):.5f}", 20, y); y += 20
            draw_text(screen, font, f"Draw every M: {draw_every_M} (D toggle, Up/Down adjust)", 20, y); y += 20
            draw_text(screen, font, f"Thermostat: {'ON' if (thermostat and step < thermostat_steps) else 'OFF'} (SPACE)", 20, y); y += 20

            if show_extra_diag:
                y += 10
                draw_text(screen, font, f"dt: {float(dt):.5f} (T/G +/-10%)", 20, y); y += 20
                draw_text(screen, font, f"density: {float(density):.3f} (+/- adjust 5%)", 20, y); y += 20
                draw_text(screen, font, f"Box L: {float(L):.3f}", 20, y); y += 20
                draw_text(screen, font, f"Particles N: {N}", 20, y); y += 20
