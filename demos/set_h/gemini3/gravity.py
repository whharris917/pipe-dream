...
GRAVITY_ENABLED = True  # Default to ON
...
def build_cell_linked_list_soa(pos_x, pos_y, cell_heads, next_idx, L, cell_size, ncell_x, ncell_y):
    ...
    for i in range(Nloc):
        x = pos_x[i]
        y = pos_y[i]
        # Clamp to avoid index errors at boundary L
        cx = int(x * inv_cell)
        cy = int(y * inv_cell)
...
# ============================================================
# Force computation: cell traversal (No PBC, with Gravity)
# ============================================================
@njit(fastmath=True)
def compute_forces_cells_model_soa(
    ...
    ncell_x, ncell_y,
    g_val, mass # Gravity arguments
):
    Nloc = pos_x.shape[0]
    for i in range(Nloc):
        force_x[i] = np.float32(0.0)
        # Initialize Y force with gravity (F = m * g)
...
# ============================================================
# Verlet pair-list building (No PBC)
# ============================================================
@njit(fastmath=True)
...
# ============================================================
# Fused Multi-step Kernel (With Reflective Walls & Gravity)
# ============================================================
@njit(fastmath=True)
def step_pairs_n(
    nsteps,
    ...
    for s in range(nsteps):
        ...
        # integrate half step with REFLECTIVE WALLS
        for i in range(Nloc):
...
# ============================================================
# Integration (cell mode fallback - No PBC)
# ============================================================
@njit(fastmath=True)
...
def main():
    ...
    pygame.display.set_caption("2D MD - Particles in a Box + Gravity")
    ...
    if VERLET_MODE_ENABLED:
        ...
        # Calculate initial gravity value
    ...
    while True:
        ...
        if draw_skip_enabled or not VERLET_MODE_ENABLED:
            ...
            draw_text(screen, font, f"Gravity: {'ON' if GRAVITY_ENABLED else 'OFF'} (G toggle)", 20, y); y += 20
            if VERLET_MODE_ENABLED:
