...
INITIAL_L = L  # Store initial size for fixed-scale visualization
...
SIM_W = 720
SIM_H = 720
UI_H = 240 # Increased extra space for UI
WINDOW_W = SIM_W
WINDOW_H = SIM_H + UI_H
...
# ============================================================
# NEW: Clamp positions when L shrinks (Safe Respawn)
# ============================================================
@njit(fastmath=True)
def enforce_box_size_soa(pos_x, pos_y, vel_x, vel_y, L, sigma):
    """
    If particles are outside the new box L, respawn them at a random location inside.
    Check to ensure they aren't placed too close to existing particles (high force).
    """
    N = pos_x.shape[0]
    # Safe distance threshold squared (0.85 * sigma)^2
    min_dist_sq = (np.float32(0.85) * sigma) ** 2
    
    for i in range(N):
        outside = False
        if pos_x[i] >= L or pos_x[i] < 0.0: outside = True
        elif pos_y[i] >= L or pos_y[i] < 0.0: outside = True
        
        if outside:
            # Respawn logic
            placed = False
            # Try finding a safe spot
            for attempt in range(100):
                rx = np.float32(random.random()) * L
                ry = np.float32(random.random()) * L
                
                # Check overlap against all other particles
                collision = False
                for j in range(N):
                    if i == j: continue
                    dx = rx - pos_x[j]
                ...
                if not collision:
                    pos_x[i] = rx
                    pos_y[i] = ry
                    # Reset velocity to avoid carrying high kinetic energy
                    vel_x[i] = 0.0
                    vel_y[i] = 0.0
                    placed = True
                    break
            
            # Fallback: Clamp to edge if we couldn't find a spot
            if not placed:
                margin = np.float32(0.05)
                if pos_x[i] < 0: pos_x[i] = margin
                elif pos_x[i] >= L: pos_x[i] = L - margin
                if pos_y[i] < 0: pos_y[i] = margin
                elif pos_y[i] >= L: pos_y[i] = L - margin
                vel_x[i] = 0.0
                vel_y[i] = 0.0
...
def force_LJ_from_r2(r2, sigma, epsilon):
    # SAFETY: Clamp r2 to avoid float32 overflow at very small distances
...
def sim_to_screen(x, y, zoom, pan_x, pan_y):
    ...
    # Base scale now relies on INITIAL_L to keep visualization "grounded"
    # regardless of the current box L
    base_scale = (float(SIM_W) - 2.0 * MARGIN) / float(INITIAL_L)
...
def main():
    ...
    global target_temp, WALL_DAMPING, gravity_accel, INITIAL_L, sigma
    ...
    # Range 10.0 to 50.0 covers typical densities
    slider_L = Slider(20, ui_start_y + 160, 200, 10, 10.0, 50.0, float(L), "Box Size (L)")
    ...
    ui_elements = [slider_temp, slider_dt, slider_damping, slider_gravity, slider_L, btn_thermostat, btn_gravity, btn_draw]
    ...
    while True:
        ...
        if abs(target_L - L) > 1e-4:
            L = target_L
            # Update density for display
            density = N / (L * L)
            # Recalculate grid
            ncell_x = max(1, int(float(L) // float(cell_size)))
            ncell_y = max(1, int(float(L) // float(cell_size)))
            # Clamp particles that are outside the new L immediately
            enforce_box_size_soa(pos_x, pos_y, vel_x, vel_y, L, sigma)
            # Force rebuild
            pair_count = 0
