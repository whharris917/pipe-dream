...
INTERACTION_J = 2.0     # Reduced slightly to allow more mixing
MU_SOURCE = 5.0         

# CRITICAL FIX:
# If this is 1.0, particles lock into crystal grids (101010).
# If this is < 1.0, they break symmetry and diffuse naturally.
MAX_MOVE_PROB = 0.5     
...
class BitwisePipe:
    ...
    def get_boltzmann_mask(self, dE_tensor, length):
        """
        Calculates move probability.
        CRITICAL CHANGE: Capped at MAX_MOVE_PROB to prevent lockstep oscillation.
        """
        if dE_tensor <= 0:
            # Even if favorable, don't move 100% of the time.
            # This creates the "Stumbling" random walk needed for diffusion.
            base_p = MAX_MOVE_PROB 
        else:
            # Uphill moves are naturally rare, but we scale them too
    ...
    def step_monte_carlo(self, chemical_potential_on):
        ...
        if chemical_potential_on:
            ...
            dE_insert = -MU_SOURCE
            # We cap injection too, to prevent creating solid crystal blocks at the source
...
def main():
    ...
    pygame.display.set_caption("Stochastic Fix Demo")
    ...
    while running:
        ...
        screen.blit(font.render("Stochastic Fix (Prob < 1.0)", True, COLOR_TEXT), (20, 50))
        screen.blit(font.render(f"MAX_MOVE_PROB: {MAX_MOVE_PROB}", True, (200, 200, 200)), (20, 80))
        screen.blit(font.render("Symmetry is broken. Fluid diffuses.", True, (200, 200, 200)), (20, 100))
