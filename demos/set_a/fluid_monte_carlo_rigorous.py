...
# --- THERMODYNAMICS ---
# Tuned for robust flow
KT = 0.5                
GRAVITY_POTENTIAL = 0.6 
INTERACTION_J = 3.0     # Strong repulsion prevents freezing
MU_SOURCE = 5.0         
DRIFT_BIAS = 0.05       # Small bias to break symmetry in horizontal pipes
...
class BitwisePipe:
    def __init__(self, length, lanes, angle_deg=0):
        ...
        # We add a tiny bias to Longitudinal Energy to ensure 
        # horizontal pipes drift Right instead of purely random walking.
        bias_x = math.sin(rad) * GRAVITY_POTENTIAL
        if abs(bias_x) < 0.01: bias_x = DRIFT_BIAS # Artificial drift for flat pipes

        self.dE_long_plus  = -bias_x
        self.dE_long_minus =  bias_x
    ...
    def step_monte_carlo(self, chemical_potential_on):
        ...
        for d in directions:
            if d == 0: # --- MOVE RIGHT (Downstream) ---
                dE_base = self.dE_long_plus
                
                # Phantom Source at Length (only if pumped)
                # We need to shift it into the frame of the pipe
                # If we are checking "Neighbor at Left", we shift neighbor RIGHT (Higher->Lower)
                # Phantom is at bit L. Shifted >> 1 is bit L-1.
                phantom_bit = (1 << (self.length - 1)) if chemical_potential_on else 0
                
                for i in range(self.lanes):
                    state = self.states[i]
                    
                    # 1. IDENTIFY NEIGHBORS (At Source Position)
                    # "Am I being pushed from upstream?"
                    # My Pos: i. Neighbor Pos: i+1.
                    # Bring Neighbor to Me: (state >> 1) | phantom
                    neighbors = (state >> 1) | phantom_bit
                    is_pushed = state & neighbors
                    is_lonely = state & (~neighbors)
                    
                    # 2. CALCULATE ACCEPTANCE (At Source Position)
                    # Pushed particles have lower dE (favorable)
                    ...
                    allowed_sources = (is_pushed & mask_pushed) | (is_lonely & mask_lonely)
                    
                    # 3. SHIFT TO DESTINATION
                    # Now we apply the move. Move is Right (>> 1).
                    allowed_moves_at_dest = allowed_sources >> 1
                    
                    # 4. COLLISION CHECK
                    # Target must be empty
                    holes = ~state
                    final_moves = allowed_moves_at_dest & holes
                    
                    # 5. VALVE CHECK & BOUNDARY
                    final_moves &= valve_blocker & self.mask
                    
                    # 6. UPDATE (Source = Moves << 1, Dest = Moves)
                    state = state ^ (final_moves << 1) ^ final_moves
                    self.states[i] = state

            elif d == 1: # --- MOVE LEFT (Upstream) ---
                dE_base = self.dE_long_minus
                
                for i in range(self.lanes):
                    state = self.states[i]
                    
                    # Neighbor is Downstream (Right, Lower Index)
                    # My Pos: i. Neighbor: i-1.
                    # Bring Neighbor to Me: (state << 1)
                    neighbors = (state << 1)
                    is_pushed = state & neighbors
                    is_lonely = state & (~neighbors)
                    ...
                    holes = ~state
                    final_moves = allowed_moves_at_dest & holes & valve_blocker & self.mask
                ...
                for i in range(self.lanes - 2, -1, -1):
                    ...
                    is_pushed = has_neighbor
                    is_lonely = src & (~has_neighbor)
                    
                    mask_pushed = self.get_boltzmann_mask(dE_base - INTERACTION_J, self.length)
                    mask_lonely = self.get_boltzmann_mask(dE_base, self.length)
                    
                    allowed_src = (is_pushed & mask_pushed) | (is_lonely & mask_lonely)
                    
                    # No shift needed for transverse logic as masks align lane-wide
                    # Target is simply 'tgt'
                    final_moves = allowed_src & (~tgt)
                    
                    self.states[i] ^= final_moves
                    self.states[i+1] |= final_moves

            elif d == 3: # --- MOVE UP ---
                dE_base = self.dE_trans_minus
                
                for i in range(self.lanes - 1):
                    src = self.states[i+1] # Moving from lower
                    tgt = self.states[i]
        ...
        if chemical_potential_on:
            ...
            dE_insert = -MU_SOURCE
            prob = 1.0 if dE_insert < 0 else math.exp(-dE_insert/KT)
...
def main():
    ...
    pygame.display.set_caption("Fixed Alignment Physics")
    ...
    while running:
        ...
        screen.blit(font.render("Corrected Alignment", True, COLOR_TEXT), (20, 50))
        screen.blit(font.render("Acceptance calculated at Source.", True, (200, 200, 200)), (20, 80))
        screen.blit(font.render("Shifted to Dest for application.", True, (200, 200, 200)), (20, 100))
