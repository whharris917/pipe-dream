...
DEFAULT_WIDTH = 1200
DEFAULT_HEIGHT = 800
PHYSICS_STEPS_PER_FRAME = 40 # Reduced steps, relying on better physics logic
...
# These replace the explicit "Speed" variables
K_GRAVITY = 0.6       # Bias from slope
K_REPULSION = 0.95    # The "Pressure": How strongly neighbors push you
K_THERMAL = 0.15      # Base random jitter (Temperature)
...
class BitwisePipe:
    def __init__(self, length, lanes, angle_deg=0):
        ...
        # Calculate Local Gravity Vectors
        rad = math.radians(angle_deg)
        self.g_long = math.sin(rad) * K_GRAVITY
        self.g_trans = math.cos(rad) * K_GRAVITY
        self.angle = angle_deg

    def reset(self):
        self.states = [0] * self.lanes

    def inject(self, probability):
        """
        The Source: A boundary condition.
        If probability is 1.0 (High Pressure), we try to force atoms in every tick.
        This naturally creates high density at the inlet.
        """
    ...
    def update(self):
        # --- 1. TRANSVERSE PHYSICS (Gravity & Leveling) ---
        
        # Downward Force: Gravity
        force_down = abs(self.g_trans)
        
        # Upward Force: Pure Thermal Jitter + Repulsion from crowded bottom
        # If the bottom is packed, K_REPULSION naturally makes particles jump up
        force_up = K_THERMAL 

        # Apply Transverse Down (Falling)
        # We perform 3 passes to simulate acceleration
        for _ in range(3):
            for i in range(self.lanes - 2, -1, -1):
                upper = self.states[i]
                lower = self.states[i+1]
                candidates = upper & (~lower) # Particle above, hole below
                
                # Gravity makes falling very likely
                move_mask = self.get_stochastic_mask(force_down + 0.1)
                falling = candidates & move_mask
                self.states[i] ^= falling
                self.states[i+1] |= falling

        # Apply Transverse Up (Mixing/Leveling)
        # Only happens via thermal energy or if squeezed
        if force_up > 0:
            for i in range(self.lanes - 1):
                ...
                move_mask = self.get_stochastic_mask(force_up)
                rising = candidates & move_mask
                self.states[i+1] ^= rising
                self.states[i] |= rising


        # --- 2. LONGITUDINAL PHYSICS (The Interaction Model) ---
        
        # Valve Logic (Physical Wall)
        ...
        for i in range(self.lanes):
            ...
            # --- CALCULATE INTERACTION MASKS ---
            
            # Left Neighbors: (state << 1)
            # If a bit has a left neighbor, it feels "Push Right" force
            has_left_neighbor = (state << 1) & state
            
            # Right Neighbors: (state >> 1)
            # If a bit has a right neighbor, it feels "Push Left" force
            has_right_neighbor = (state >> 1) & state
            
            # --- MOVE RIGHT ---
            # Base motivation: Thermal + Gravity + Repulsion from Left
            # We construct a probability mask dynamically based on neighbors
            
            # 1. Base Mask (Thermal + Gravity)
            base_prob_right = K_THERMAL + self.g_long
            mask_base_right = self.get_stochastic_mask(base_prob_right)
            
            # 2. Repulsion Mask (Triggered by neighbors)
            mask_repulsion = self.get_stochastic_mask(K_REPULSION)
            
            # 3. Combine
            # If you have a neighbor, use High Prob (Repulsion). 
            # If not, use Low Prob (Drift).
            # This logic creates the "Pressure Wave" emergent behavior.
            final_mask_right = (has_left_neighbor & mask_repulsion) | (~has_left_neighbor & mask_base_right)
            ...
            # --- MOVE LEFT ---
            # Base motivation: Thermal - Gravity + Repulsion from Right
            
            base_prob_left = K_THERMAL - self.g_long
            mask_base_left = self.get_stochastic_mask(base_prob_left)
            
            # Repulsion from right side pushes us left
            # Using same K_REPULSION constant
            
            final_mask_left = (has_right_neighbor & mask_repulsion) | (~has_right_neighbor & mask_base_left)
...
def handle_mid_junction(main, branch, junction_pos):
    ...
    for i in range(NUM_LANES):
        if (main.states[i] >> junction_bit_idx) & 1:
            # Gravity check: 90% chance to fall into the hole
...
def main():
    ...
    pygame.display.set_caption("Thermodynamic Emergent Flow")
    ...
    while running:
        ...
        for _ in range(PHYSICS_STEPS_PER_FRAME):
            # SOURCE: Just a boundary probability!
            # High Pressure Source = 100% Injection chance
            if pumping:
                main_pipe.inject(probability=1.0)
            
            # UPDATES: No 'pump_is_on' passed. 
            # Flow emerges from density gradients.
            main_pipe.update()
            branch_pipe.update()
            bottom_pipe.update()
        ...
        screen.blit(font.render("Emergent Thermodynamic Flow", True, COLOR_TEXT), (20, 50))
        screen.blit(font.render("Forces: Gravity, Thermal, Repulsion", True, (200, 200, 200)), (20, 80))
        screen.blit(font.render("No global pressure variable used.", True, (200, 200, 200)), (20, 100))
