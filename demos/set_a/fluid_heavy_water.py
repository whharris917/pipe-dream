...
# "Heavy" Fluid Settings
K_GRAVITY = 0.6       
K_REPULSION = 0.98    # Very High: Atoms hate touching neighbors -> Pushes flow fast
K_THERMAL = 0.01      # Very Low: Atoms rarely jump up -> Reduces "mist/sloshing"
...
class BitwisePipe:
    ...
    def update(self):
        # --- 1. TRANSVERSE PHYSICS ---
        
        force_down = abs(self.g_trans)
        
        # Reduced Upward Jiggle (As requested)
        force_up = K_THERMAL 

        # Gravity (Down) - 3 Passes for weight
        for _ in range(3):
            for i in range(self.lanes - 2, -1, -1):
                upper = self.states[i]
                lower = self.states[i+1]
                candidates = upper & (~lower)
                move_mask = self.get_stochastic_mask(force_down + 0.2)
        ...
        # --- 2. LONGITUDINAL PHYSICS (Local Repulsion) ---
        ...
        for i in range(self.lanes):
            ...
            # --- MOVE RIGHT ---
            # Base = Thermal + Gravity
            # Push = Repulsion from Left
            ...
            # --- MOVE LEFT ---
            # Base = Thermal - Gravity (Usually negative in high gravity sections)
            # Push = Repulsion from Right
...
def main():
    ...
    pygame.display.set_caption("Heavy Water Demo")
    ...
    while running:
        ...
        screen.blit(font.render("Heavy Water / Low Temperature", True, COLOR_TEXT), (20, 50))
        screen.blit(font.render("Thermal Jitter: 0.01", True, (200, 200, 200)), (20, 80))
        screen.blit(font.render("Repulsion: 0.98", True, (200, 200, 200)), (20, 100))
