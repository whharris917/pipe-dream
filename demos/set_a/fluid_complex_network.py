...
BOTTOM_PIPE_LENGTH = 150  # New Segment
NUM_LANES = 16            
CELL_SIZE = 5             
...
class BitwisePipe:
    ...
    def update(self, pump_is_on):
        ...
        for i in range(self.lanes):
            ...
            combined_mask_right = (r_active & pressure_zone_mask) | (r_passive & passive_zone_mask)
            
            candidates = (state >> 1) & (~state) & self.mask
            valid_moves = candidates & combined_mask_right & valve_wall_mask
            state = state ^ valid_moves ^ (valid_moves << 1)

            r_left = self.get_stochastic_mask(f_left)
            candidates = (state << 1) & (~state) & self.mask
            valid_moves = candidates & r_left & valve_wall_mask
            state = state ^ valid_moves ^ (valid_moves >> 1)
...
def handle_junction(source, dest):
    # Standard source->dest transfer
    # Source Bit: Last bit (0)
    # Dest Bit: First bit (Length-1)
    
    # NOTE: Bit index 0 is the physical "End" of the pipe in our bitwise model.
    source_exit_idx = 0 
    dest_entry_idx = dest.length - 1
    
    # 1. Identify all empty lanes in the dest pipe
    empty_dest_lanes = []
    ...
    for i in range(NUM_LANES):
        ...
        if (source.states[i] >> source_exit_idx) & 1:
            ...
            # Momentum/Connection check
            # For 90 degree or sharp turns, we might want some resistance?
            # Keeping it simple: 90% transfer rate
...
def handle_mid_junction(main, branch, junction_pos):
    # Special handler for T-Junctions (like the first one)
...
def main():
    ...
    pygame.display.set_caption("Complex Network Demo")
    ...
    # Calculate end of diagonal pipe to place start of bottom pipe
    ...
    while running:
        ...
        for _ in range(PHYSICS_STEPS_PER_FRAME):
            ...
            branch_pipe.update(pump_is_on=True) # Always allow drain
            bottom_pipe.update(pump_is_on=True)
            
            # Topology Update
            handle_mid_junction(main_pipe, branch_pipe, JUNCTION_POS)
            handle_junction(branch_pipe, bottom_pipe) # End of branch -> Start of bottom
        ...
        screen.blit(font.render("3-Stage Network", True, COLOR_TEXT), (20, 50))
        screen.blit(font.render("1. Main (Horz) -> Branch (Diag)", True, (200, 200, 200)), (20, 70))
        screen.blit(font.render("2. Branch (Diag) -> Bottom (Horz)", True, (200, 200, 200)), (20, 90))
        screen.blit(font.render("Watch the 'Back-Up' when bottom fills!", True, (200, 200, 200)), (20, 110))
