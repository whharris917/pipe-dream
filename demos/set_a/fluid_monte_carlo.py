...
# FLUID PROPERTIES
# These control the "Personality" of the fluid
FLOW_SPEED = 0.90         # Base speed of unpressurized water
PRESSURE_POWER = 0.98     # Speed when pushed by upstream neighbors
LEVELING_SPEED = 0.80     # Speed of horizontal spreading (slumping)
GRAVITY_STRENGTH = 0.70   # Speed of falling
...
class BitwisePipe:
    def __init__(self, length, lanes, angle_deg=0):
        ...
        # We store vectors to know if we are horizontal or vertical
        self.is_horizontal = (abs(math.sin(rad)) < 0.1)
    ...
    def update(self, pump_is_on):
        ...
        # --- 1. TRANSVERSE (GRAVITY & MIXING) ---
        # "Gravity" works perpendicular to flow
        # In a horizontal pipe, this packs fluid to the bottom.
        ...
        for _ in range(3):
            for i in range(self.lanes - 2, -1, -1):
                ...
                # Gravity is strong
                move_mask = self.get_stochastic_mask(GRAVITY_STRENGTH)
                falling = candidates & move_mask
                self.states[i] ^= falling
                self.states[i+1] |= falling

        # Diffusion (Up)
        # This allows fluid to climb out of the bottom lane and "tumble" forward
        # Critical for the diagonal pipe to feed the bottom pipe
        for i in range(self.lanes - 1):
            upper = self.states[i]
            lower = self.states[i+1]
            candidates = lower & (~upper)
            
            move_mask = self.get_stochastic_mask(LEVELING_SPEED * 0.5) 
        ...
        if self.valve_active and not self.valve_open:
            ...
            # Pressure only exists upstream of closed valve
        ...
        for i in range(self.lanes):
            ...
            # --- CALCULATE UPSTREAM DENSITY (PRESSURE) ---
            # We look 1, 2, and 3 bits to the Left (Upstream).
            # If these are full, the pressure on the current bit is high.
            p1 = (state << 1) & state
            p2 = (state << 2) & state
            p3 = (state << 3) & state
            
            # This mask identifies atoms that are being "pushed" from the left
            being_pushed = p1 | p2 | p3
            
            # --- MOVE RIGHT (Downstream) ---
            
            # 1. High Pressure Flow (Pump Driven or Clump Driven)
            # If pushed, move with PRESSURE_POWER.
            # If not pushed, move with FLOW_SPEED.
            mask_push = self.get_stochastic_mask(PRESSURE_POWER)
            mask_drift = self.get_stochastic_mask(FLOW_SPEED)
            
            # Combine logic:
            # Active Zone: Use Pushed/Drift logic.
            # Passive Zone: Use Drift only (or Gravity if sloped).
            
            if self.is_horizontal:
                # Horizontal: Move if pushed OR if drifting
                eff_mask_active = (being_pushed & mask_push) | (~being_pushed & mask_drift)
                # Passive: Only drift (slower)
                eff_mask_passive = mask_drift 
            else:
                # Sloped: Gravity accelerates everything to max speed
                eff_mask_active = self.mask
                eff_mask_passive = self.mask # Falling water falls fast

            # Compose final Right Mask
            final_mask_right = (eff_mask_active & pressure_zone_mask) | \
                               (eff_mask_passive & passive_zone_mask)
            
            candidates = (state >> 1) & (~state) & self.mask
            valid = candidates & final_mask_right & valve_wall_mask
            state = state ^ valid ^ (valid << 1)

            # --- MOVE LEFT (Leveling / Back-Flow) ---
            # Only happens in passive zones or if explicitly leveling
            # Gravity opposes this in sloped pipes.
            
            level_speed = LEVELING_SPEED
            if not self.is_horizontal:
                level_speed = 0.05 # Hard to move upstream against gravity
            
            mask_left = self.get_stochastic_mask(level_speed)
            
            candidates = (state << 1) & (~state) & self.mask
            # Only allow back-flow in passive zones (can't swim upstream against pump)
            # OR if we want hydrostatic leveling everywhere.
            # Let's restrict it to passive zones for clear "Pump" visualization.
            valid = candidates & mask_left & valve_wall_mask & passive_zone_mask
...
def main():
    ...
    pygame.display.set_caption("Pressure Density Flow")
    ...
    while running:
        ...
        for _ in range(PHYSICS_STEPS_PER_FRAME):
            ...
            # Use pumping flag for main pipe
            main_pipe.update(pump_is_on=pumping)
            
            # The downstream pipes are NOT pumped.
            # They flow based on their own internal density gradients.
        ...
        screen.blit(font.render("Density-Driven Flow", True, COLOR_TEXT), (20, 50))
        screen.blit(font.render("Upstream clumps push downstream.", True, (200, 200, 200)), (20, 80))
        screen.blit(font.render("No symmetric cancellation.", True, (200, 200, 200)), (20, 100))
