...
N_PARTICLES = 10000  # Slightly reduced to fit nicely in the bowl
GRAVITY = 0.5
MAX_VEL = 4.0       
FRICTION = 0.96     
OVERLAP_FORCE = 10.0 
...
def init_system(width, height, n_particles):
    ...
    Spawns particles in the air above the bowl.
    """
    ...
    for i in range(n_particles):
        ...
        vel_y[i] = np.random.uniform(0.0, 1.0) # Downward bias
                
    return pos_x, pos_y, vel_x, vel_y

@njit(fastmath=True)
def update_physics(pos_x, pos_y, vel_x, vel_y, head, next_particle, obstacles, width, height):
    """
    ...
    for i in order:
        ...
        for dy in range(-r, r + 1):
            ...
            for dx in range(-r, r + 1):
                ...
                # Force from Obstacles (Short range repulsion from walls)
                # This helps prevent sticking/tunneling
                if obstacles[ny, nx] == 1:
                    dist_sq = dx*dx + dy*dy
                    if dist_sq <= 2:
                        fx -= dx * 0.5 # Push back from wall
                        fy -= dy * 0.5
        ...
        # --- 4. Discrete Movement (Obstacle Aware) ---
        dx_step = 0
        dy_step = 0
...
def render(pixels, pos_x, pos_y, obstacles, fluid_color, bg_color, obs_color):
    ...
    # We can iterate the grid or fill entire array. 
    # For speed, let's iterate.
    for y in range(h):
        for x in range(w):
            if obstacles[y, x] == 1:
                pixels[x, y] = c_obs
