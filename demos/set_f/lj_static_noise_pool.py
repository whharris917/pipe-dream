...
def update_physics(pos_x, pos_y, vel_x, vel_y, accum_x, accum_y, head, next_particle, obstacles, width, height, noise_pool, noise_offset_x, noise_offset_y):
    ...
    for i in order:
        ...
        # Instead of calculating random numbers, we fetch from the pre-calc pool
        # using bitwise masking for wrapping.
        
        # Apply Thermal Noise X
        idx_x = (i + noise_offset_x) & NOISE_MASK
        vel_x[i] += noise_pool[idx_x]
        
        # Apply Thermal Noise Y (use different offset to decouple dimensions)
...
class Simulation:
    def __init__(self):
        ...
        # Create Static Noise Pool (Pre-calculated Gaussian distribution)
    ...
    def update(self):
        ...
        # This keeps the noise dynamic without re-calculating the distribution
