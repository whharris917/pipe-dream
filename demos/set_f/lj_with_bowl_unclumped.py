...
N_PARTICLES = 10000  
GRAVITY = 0.25      # Stronger gravity to dominate attraction
MAX_VEL = 5.0       
FRICTION = 0.99     # Very low friction to allow flowing
OVERLAP_FORCE = 2.0 
TEMPERATURE = 0.1   
    ...
    for dy in range(-radius, radius + 1):
        for dx in range(-radius, radius + 1):
            ...
            if dist_sq <= 2:
                ...
                # CRITICAL FIX: Attraction must be weak enough that 
                # 4-5 neighbors cannot lift a particle against gravity (0.25).
                # 0.02 * 5 = 0.1 < 0.25. (Safe)
                force = -0.02 
...
def create_bowl_obstacles(width, height):
    ...
    1 = Obstacle (Wall), 0 = Empty (Air).
    """
    obstacles = np.zeros((height, width), dtype=np.int32)
    cx = width // 2
    cy = height // 2 - 10
    radius = width // 2 - 15  # Radius of the bowl interior

    for y in range(height):
        for x in range(width):
            # Fill the bottom half to create a 'ground'
            if y > cy:
                obstacles[y, x] = 1
                
            # Carve out a semi-circle to make the bowl
...
def init_system(width, height, n_particles):
    ...
    Now includes accumulators for sub-pixel movement precision.
    """
    ...
    # Spawn box centered above the bowl
    spawn_w = width // 2
    spawn_h = height // 3
...
@njit(fastmath=True)
def update_physics(pos_x, pos_y, vel_x, vel_y, accum_x, accum_y, head, next_particle, obstacles, width, height):
    """
    Discrete Particle Dynamics allowing overlap + Obstacle Collision.
    Uses sub-pixel accumulation to prevent freezing.
    """
    n = len(pos_x)
    ...
    for i in order:
        ...
        # Apply Thermal Noise
        vel_x[i] += np.random.normal(0, TEMPERATURE)
        vel_y[i] += np.random.normal(0, TEMPERATURE)
        ...
        # Instead of probabilistic movement, we accumulate sub-pixel error.
        # This ensures gravity ALWAYS eventually moves a particle.
        
        # X Axis
        accum_x[i] += vel_x[i]
        steps_x = int(accum_x[i])
        accum_x[i] -= steps_x # Keep the remainder
        ...
        if steps_x != 0:
            ...
            for _ in range(count):
                ...
                if 0 <= nx < width and obstacles[py, nx] == 0:
                    ...
                    vel_x[i] *= -0.5
                    accum_x[i] = 0 # Clear accumulator on bounce
                    break
        ...
        if steps_y != 0:
            ...
            for _ in range(count):
                ...
                if 0 <= ny < height and obstacles[ny, px] == 0:
                    ...
                    vel_y[i] *= -0.5
                    accum_y[i] = 0 
                    vel_x[i] *= 0.9 # Friction on floor
                    break

@njit(fastmath=True)
def render(pixels, pos_x, pos_y, obstacles, fluid_color, bg_color, obs_color):
    """
    Render obstacles and particles.
    """
    # 1. Clear & Draw Obstacles
    h, w = obstacles.shape
...
class Simulation:
    ...
    def draw(self):
        render(self.pixel_array, self.pos_x, self.pos_y, self.obstacles, FLUID_COLOR, BG_COLOR, OBSTACLE_COLOR)
