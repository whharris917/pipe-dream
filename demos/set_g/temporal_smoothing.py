...
def render_fluid_density(pixels, pos_x, pos_y, raw_grid, density_history, frame_idx, static_bg, width, height, fluid_color, bg_color):
    ...
    2. Spatially smooth raw_grid and store in density_history[frame].
    3. Temporally average density_history across 10 frames.
    4. Threshold.
    """
    # 1. Reset Raw Grid
    raw_grid[:] = 0.0
    ...
    for i in range(len(pos_x)):
        ...
        if 2 < px < width-3 and 2 < py < height-3:
            ...
            # Inner Ring
            raw_grid[px+1, py] += 6.0
            raw_grid[px-1, py] += 6.0
            raw_grid[px, py+1] += 6.0
            raw_grid[px, py-1] += 6.0
            
            raw_grid[px+1, py+1] += 4.0
            raw_grid[px-1, py-1] += 4.0
            raw_grid[px-1, py+1] += 4.0
            raw_grid[px+1, py-1] += 4.0
            
            # Outer Ring
            raw_grid[px+2, py] += 2.0
            raw_grid[px-2, py] += 2.0
            raw_grid[px, py+2] += 2.0
            raw_grid[px, py-2] += 2.0
            
            raw_grid[px+2, py+1] += 1.0
            raw_grid[px+2, py-1] += 1.0
            raw_grid[px-2, py+1] += 1.0
            raw_grid[px-2, py-1] += 1.0
            
            raw_grid[px+1, py+2] += 1.0
            raw_grid[px-1, py+2] += 1.0
            raw_grid[px+1, py-2] += 1.0
            raw_grid[px-1, py-2] += 1.0
    ...
    for y in prange(2, height-2):
        for x in range(2, width-2):
            ...
            # A. Spatial Smoothing (5x5 Area Summation of raw_grid)
            d_smooth = 0.0
            for dy in range(-2, 3):
                for dx in range(-2, 3):
            ...
            # D. Thresholding
            # Avg = d_total / 10.0. 
            # Previous threshold was 150.0 per frame. 
            # So d_total must be > 150.0 * 10 = 1500.0
            if d_total > 1500.0: 
                pixels[x, y] = c_fluid
            else:
                pixels[x, y] = bg_val
