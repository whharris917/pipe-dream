...
MAX_VEL = 5.0           # Higher velocity cap to allow falling
FRICTION = 0.7          # Air resistance / Drag
OVERLAP_FORCE = 3.0     # Stronger repulsion to prevent collapse
TEMPERATURE = 0.0       # High thermal noise to prevent freezing
...
# Radius 0 = No smoothing (1x1), Radius 1 = 3x3 kernel, Radius 2 = 5x5 kernel
SMOOTHING_RADIUS = 0    

# Thresholding
# Lowered to 5.0 because with Radius 0, we don't sum neighbors, so density is much lower.
THRESHOLD_BASE = 1.0
ROLLING_FRAMES = 1      # No averaging, show raw physics state
            ...
            for dx in range(-KERNEL_RADIUS, KERNEL_RADIUS + 1):
                ...
                while j != -1:
                    if i != j:
                        if dx == 0 and dy == 0:
                            pass
                            # Random kick scaled by TEMPERATURE for exact overlaps
                            rand_angle = np.random.uniform(0, 6.28)
                            fx += np.cos(rand_angle) * OVERLAP_FORCE * TEMPERATURE
                            fy += np.sin(rand_angle) * OVERLAP_FORCE * TEMPERATURE
                        else:
