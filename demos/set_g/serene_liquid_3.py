...
MAX_VEL = 10.0           # Higher velocity cap to allow falling
FRICTION = 0.9          # Heavy damping to stop jitter
OVERLAP_FORCE = 0.0     # ZERO overlap kick (User request)
TEMPERATURE = 0.001     # Absolute zero

# 5. Force Kernel Configuration (Lattice Potentials)
KERNEL_RADIUS = 100
FORCE_REPULSION_DIST_SQ = 0.25
FORCE_REPULSION_STRENGTH = 0.5  
FORCE_ATTRACTION_DIST_SQ = 9
FORCE_ATTRACTION_STRENGTH = -1.0

# 6. Obstacle & Wall Interactions
WALL_REPULSION_DIST_SQ = 1
WALL_REPULSION_STRENGTH = 0.001
WALL_BOUNCE_FACTOR = -0.0       
FLOOR_FRICTION_FACTOR = 0.9     
        ...
        for dy in range(-KERNEL_RADIUS, KERNEL_RADIUS + 1):
            ...
            for dx in range(-KERNEL_RADIUS, KERNEL_RADIUS + 1):
                ...
                while j != -1:
                    if i != j:
                        if dx == 0 and dy == 0:
                            # 0.0 force when overlapping to test pure density physics
                            fx += 0.0
                            fy += 0.0
                        else:
