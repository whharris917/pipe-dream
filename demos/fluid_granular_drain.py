...
GLOBAL_GRAVITY = 0.50      
BASE_DIFFUSION = 0.70      # Increased slightly to ensure good drain-back behavior
...
class BitwisePipe:
    ...
    def update(self, pump_is_on):
        # 1. Transverse Physics (Gravity & Mixing)
...
def handle_junction(main, branch):
    # FIXED LOGIC: CONSERVATION OF MASS
    junction_bit_idx = main.length - 1 - JUNCTION_POS
    branch_inlet_idx = branch.length - 1
    
    # 1. Identify all empty lanes in the branch pipe
    empty_branch_lanes = []
    for i in range(NUM_LANES):
        if not ((branch.states[i] >> branch_inlet_idx) & 1):
            empty_branch_lanes.append(i)
    
    # Randomize to simulate chaotic splashing
    random.shuffle(empty_branch_lanes)
    
    # 2. Iterate through Main lanes and transfer IF space exists
    for i in range(NUM_LANES):
        # Check if Main has atom at junction
        if (main.states[i] >> junction_bit_idx) & 1:
            
            # Momentum Check: 10% chance to skip the hole and stay in Main
            if random.random() > 0.90:
                continue

            # If Branch is full, do nothing (Atom stays in Main -> Overflow behavior)
            if not empty_branch_lanes:
                break 
            
            # Transfer!
            target_lane = empty_branch_lanes.pop()
...
def main():
    ...
    pygame.display.set_caption("Final Fluid Simulation")
    ...
    while running:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.button == 1:
                    ...
                    sx, sy = camera.world_to_screen(w_valve_x, world_main_y)
        ...
        for _ in range(PHYSICS_STEPS_PER_FRAME):
            ...
            branch_pipe.update(pump_is_on=True)
        ...
        screen.blit(font.render("Mass Conservation Fixed!", True, COLOR_TEXT), (20, 50))
        screen.blit(font.render("Try closing the valve:", True, (200, 200, 200)), (20, 80))
        screen.blit(font.render("Atoms will level out and drain back.", True, (200, 200, 200)), (20, 100))
