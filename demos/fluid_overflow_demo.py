...
def handle_junction(main, branch):
    ...
    # 1. IDENTIFY FALLING CANDIDATES (With Momentum Check)
    candidates = []
    for i in range(NUM_LANES):
        if (main.states[i] >> junction_bit_idx) & 1:
            # 90% chance to fall, 10% chance to SKIP due to momentum
    ...
    # 2. IDENTIFY EMPTY SPOTS IN BRANCH
    empty_branch_lanes = []
    for i in range(NUM_LANES):
        if not ((branch.states[i] >> branch_inlet_idx) & 1):
            empty_branch_lanes.append(i)
    
    # Randomize insertion order to simulate splashing
    random.shuffle(empty_branch_lanes)
    
    # 3. EXECUTE TRANSFER (Only if space exists)
    for main_lane in candidates:
        if not empty_branch_lanes:
            # Branch is FULL. 
            # Do NOT remove from Main. Let it flow past.
            break 
        ...
        main.states[main_lane] &= ~(1 << junction_bit_idx)
...
def main():
    ...
    pygame.display.set_caption("Fluid Overflow Demo")
    ...
    while running:
        ...
        screen.blit(font.render("Observe:", True, (200, 200, 200)), (20, 80))
        screen.blit(font.render("1. Some particles skip the hole (Momentum)", True, (200, 200, 200)), (20, 100))
        screen.blit(font.render("2. When pipe fills, flow continues right", True, (200, 200, 200)), (20, 120))
