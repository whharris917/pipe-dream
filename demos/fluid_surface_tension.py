...
# --- THERMODYNAMICS ---
# COHESIVE FLUID SETTINGS
KT = 0.2                
GRAVITY_POTENTIAL = 0.9

# The New Physics:
INTERACTION_REPULSION = 0.01 # ZERO (No pressure pushing apart)
SURFACE_TENSION_K = 1.2      # POSITIVE (Penalty for empty neighbors / Reward for touching)

MU_SOURCE = 5.0              # Needs high injection pressure to overcome the tension at the inlet
...
class BitwisePipe:
    ...
    def step_monte_carlo(self, chemical_potential_on):
        ...
        for d in directions:
            ...
            # We calculate dE based on losing/gaining neighbors.
            # Breaking a bond (Leaving a neighbor) = +K (Cost)
            # Forming a bond (Joining a neighbor)  = -K (Benefit)
            
            if d == 0: # Right
                dE_grav = self.dE_long_plus
                ...
                for i in range(self.lanes):
                    ...
                    # 1. LONGITUDINAL NEIGHBORS
                    # Neighbor Left (Upstream)
                    n_left = (state >> 1) | phantom_bit
                    # Neighbor Right (Downstream - at Dest+1)
                    # Dest is i-1. Dest+1 is i-2.
                    # We check if (state >> 2) has bits.
                    n_right_of_dest = (state >> 2)
                    
                    # Breaking Bond (Leaving Left):
                    is_breaking = state & n_left
                    
                    # Forming Bond (Approaching Right):
                    is_forming = state & n_right_of_dest
                    
                    # 2. TRANSVERSE NEIGHBORS (Shear Tension)
                    # Check lane above/below at Source vs Dest
                    # Ideally we check these, but let's start with Longitudinal Tension
                    # to prevent "plug flow" stalling.
                    ...
                    # We have 4 permutations of Break/Form. We construct masks.
                    
                    # Case A: Break & !Form (Cost +K)
                    mask_cost = self.get_boltzmann_mask(dE_grav + SURFACE_TENSION_K, self.length)
                    case_cost = is_breaking & (~is_forming)
                    
                    # Case B: !Break & Form (Benefit -K)
                    mask_benefit = self.get_boltzmann_mask(dE_grav - SURFACE_TENSION_K, self.length)
                    case_benefit = (~is_breaking) & is_forming
                    
                    # Case C: Neutral (Break & Form OR !Break & !Form) (Net 0)
                    mask_neutral = self.get_boltzmann_mask(dE_grav, self.length)
                    case_neutral = ~(case_cost | case_benefit)
                    
                    # Combine
                    acceptance = (case_cost & mask_cost) | \
                                 (case_benefit & mask_benefit) | \
                                 (case_neutral & mask_neutral)
                ...
                for i in range(self.lanes):
                    ...
                    # Neighbors
                    n_right = (state << 1)
                    n_left_of_dest = (state << 2)
                    
                    is_breaking = state & n_right
                    is_forming  = state & n_left_of_dest
                ...
                for i in range(self.lanes - 2, -1, -1):
                    ...
                    # Vertical Bonds
                    # Leaving Above (i-1)
                    has_above = (self.states[i-1] & src) if i > 0 else 0
                    # Joining Below (i+2) - Check Dest+1
                    has_below_dest = (self.states[i+2] & src) if i < self.lanes - 2 else 0
                    
                    is_breaking = has_above
                    is_forming  = has_below_dest
                    ...
                    acceptance = (case_cost & mask_cost) | (case_benefit & mask_benefit) | (case_neutral & mask_neutral)
                ...
                for i in range(self.lanes - 1):
                    ...
                    # Leaving Below (i+2)
                    has_below = (self.states[i+2] & src) if i < self.lanes - 2 else 0
                    # Joining Above (i-1)
                    has_above_dest = (self.states[i-1] & src) if i > 0 else 0
                    
                    is_breaking = has_below
                    is_forming  = has_above_dest
        ...
        if chemical_potential_on:
            ...
            dE_insert = -MU_SOURCE
            # Source must overcome internal pressure/tension to enter
...
def main():
    ...
    pygame.display.set_caption("Surface Tension Demo")
    ...
    while running:
        ...
        screen.blit(font.render("Surface Tension Only", True, COLOR_TEXT), (20, 50))
        screen.blit(font.render("Repulsion = 0. Cohesion = 1.2", True, (200, 200, 200)), (20, 80))
        screen.blit(font.render("Notice beading and droplets.", True, (200, 200, 200)), (20, 100))
